...
const UI = {
    ...
    /**
     * Update the map legend for the current step.
     * Shows marker types relevant to the step's active layers.
    ...
    updateLegend(stepIndex) {
        ...
        // Define legend items per step range
        const coreLegend = [
            { label: 'Base map', color: '#6e7073', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>' },
        ];
        ...
        if (stepIndex >= 1 && stepIndex <= 2) {
            stepItems = [
                { label: 'Resources', color: '#ff3b30', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5C6 11.1 5 13 5 15a7 7 0 0 0 7 7z"/></svg>' },
                { label: 'Energy', color: '#ff9500', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><circle cx="12" cy="12" r="4"/><path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M6.34 17.66l-1.41 1.41M19.07 4.93l-1.41 1.41"/></svg>' },
            ];
        } else if (stepIndex >= 3 && stepIndex <= 4) {
            stepItems = [
                { label: 'Government', color: '#34c759', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><line x1="3" x2="21" y1="22" y2="22"/><line x1="6" x2="6" y1="18" y2="11"/><line x1="10" x2="10" y1="18" y2="11"/><line x1="14" x2="14" y1="18" y2="11"/><line x1="18" x2="18" y1="18" y2="11"/><polygon points="12 2 20 7 4 7"/></svg>' },
                { label: 'Corporate sites', color: '#007aff', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18Z"/><path d="M6 12H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2"/><path d="M18 9h2a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-2"/></svg>' },
            ];
        } else if (stepIndex >= 5 && stepIndex <= 8) {
            stepItems = [
                { label: 'Science park', color: '#ff3b30', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M10 2v7.527a2 2 0 0 1-.211.896L4.72 20.55a1 1 0 0 0 .9 1.45h12.76a1 1 0 0 0 .9-1.45l-5.069-10.127A2 2 0 0 1 14 9.527V2"/><path d="M8.5 2h7"/></svg>' },
                { label: 'Development zones', color: '#ff3b30', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>' },
                { label: 'Infrastructure', color: '#5ac8fa', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><circle cx="6" cy="19" r="3"/><path d="M9 19h8.5a3.5 3.5 0 0 0 0-7h-11a3.5 3.5 0 0 1 0-7H15"/><circle cx="18" cy="5" r="3"/></svg>' },
            ];
        } else if (stepIndex >= 9 && stepIndex <= 10) {
            stepItems = [
                { label: 'Investment zones', color: '#fbb931', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>' },
                { label: 'Properties', color: '#ff9500', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>' },
            ];
        } else {
        ...
        const allItems = [...coreLegend, ...stepItems];
            ...
            <div class="legend-items">
                ${allItems.map(item => `
                    <div class="legend-item">
                        <div class="legend-marker" style="background: ${item.color};">
                            ${item.icon}
                        </div>
