/**
 * Traditional Chinese (Taiwan) translations.
 *
 * Tone: professional, targeting experienced real estate professionals.
 * Terminology follows Taiwan industry conventions.
 *
 * Key = English source text. Value = zh-TW translation.
 */

export default {
  // =====================================================================
  // UI Chrome (index.html, templates.js)
  // =====================================================================

  "Kumamoto Investment Opportunities": "熊本投資機會",
  "Skip to main content": "跳至主要內容",
  "Interactive map of Kumamoto investment region": "熊本投資區域互動地圖",
  "Journey progress": "導覽進度",
  "Map time view": "地圖時序檢視",
  Present: "現況",
  Future: "未來",
  "Journey guide": "投資導覽",
  "Go back": "返回",
  "Close chatbox": "關閉對話框",
  "Toggle map layers": "切換地圖圖層",
  "Reopen guide": "重新開啟導覽",
  "Data layers": "資料圖層",
  "Toggle camera debug": "切換鏡頭除錯",
  Camera: "鏡頭",
  "Copy camera values": "複製鏡頭參數",
  "Toggle QA reporter": "切換 QA 回報",
  "QA reporter": "QA 回報工具",
  "Copy all unsolved issues": "複製所有未解決問題",
  "Describe the issue...": "描述問題...",
  "Add issue": "新增問題",
  "Toggle camera explorer": "切換鏡頭探索",
  "Camera explorer": "鏡頭探索工具",
  "Switch to 3D": "切換至 3D",
  "3D buildings": "3D 建築",
  Labels: "標籤",
  Terrain: "地形",
  "Copy config": "複製設定",
  "Toggle step jumper": "切換步驟跳轉",
  "Step jumper": "步驟跳轉工具",
  "Toggle dashboard": "切換儀表板",
  Dashboard: "儀表板",
  "Go to dashboard home": "返回儀表板首頁",
  "Close dashboard": "關閉儀表板",
  "Close gallery": "關閉圖庫",
  "Document viewer": "文件檢視器",
  "Property image expanded view": "物件圖片放大檢視",
  "Close image": "關閉圖片",
  "Evidence preview": "佐證資料預覽",
  "Close preview": "關閉預覽",
  "Ask Me Anything About Kumamoto": "熊本投資相關問題",
  "What are the risks?": "有哪些風險？",
  "How do I invest?": "如何進行投資？",
  "Ask about Kumamoto...": "詢問熊本相關問題...",
  "Type your question": "輸入您的問題",
  "Send message": "送出訊息",
  "Download summary": "下載摘要",
  Continue: "繼續",
  "Start Journey": "開始導覽",
  "See the Future": "檢視未來願景",
  "Sample data · Q1 2026": "範例資料 · 2026 年第一季",

  // =====================================================================
  // Steps (steps.js)
  // =====================================================================

  Resources: "資源條件",
  "Water and power infrastructure": "水資源與電力基礎設施",
  "Water resources": "水資源",
  "Power resources": "電力資源",
  "Strategic location": "亞洲戰略位置",
  "Kyushu position in Asia": "九州的亞洲樞紐地位",
  "Government support": "政府支持",
  "National to local commitment": "從中央到地方的全面承諾",
  "Central government": "中央政府",
  "Prefectural government": "縣級政府",
  "Local municipalities": "地方自治體",
  "Corporate investment": "企業投資",
  "Seven major players": "七大核心企業",
  "Science park and grand airport": "科學園區與大空港計劃",
  "Development zones and airport connectivity": "開發區與機場連結",
  "Science park": "科學園區",
  "Government zone plan": "政府園區規劃",
  "Kikuyo long-term plan": "菊陽町中長期計劃",
  "Ozu long-term plan": "大津町中長期計劃",
  "Grand airport concept": "大空港構想",
  "Airport access": "機場連結",
  "New railway stations": "新設鐵路車站",
  "Road extensions": "道路延伸工程",
  "10-20 minute concept": "10-20 分鐘生活圈構想",
  "Education and talent pipeline": "教育與人才培育",
  "Universities, training, and employment": "大學、培訓與就業",
  Universities: "大學",
  "Employment data": "就業數據",
  "Future outlook": "未來展望",
  "Composite 2030+ vision": "2030+ 複合願景",
  "Investment opportunity zones": "投資機會區域",
  "Three zones in the silicon triangle": "矽三角的三大區域",
  "Central city": "市中心區域",
  "Middle zone": "中間地帶",
  JASM: "JASM",
  Properties: "投資物件",
  "Investment opportunities": "投資機會",
  "Kikuyo properties": "菊陽物件",
  "Ozu properties": "大津物件",
  "Journey complete": "導覽完成",
  "Summary and Q&A": "摘要與問答",
  Evidence: "佐證資料",
  Support: "支持力度",
  Investment: "投資概況",
  Companies: "企業資訊",
  Overview: "總覽",
  Education: "教育",
  Employment: "就業",
  Plans: "規劃",
  Timeline: "時程",
  Zones: "區域",
  Metrics: "指標",
  "Truth engine": "投資分析",
  Financials: "財務試算",
  Images: "圖片",
  "Future outlook": "未來展望",

  // =====================================================================
  // App.js Narratives
  // =====================================================================

  "Kumamoto investment guide": "熊本投資導覽",
  "Explore the map and use the data layers to learn about investment opportunities in Kumamoto's semiconductor corridor.":
    "探索地圖並運用資料圖層，深入了解熊本半導體走廊的投資機會。",
  "Semiconductor fabs need <strong>10 million gallons of water daily</strong> and enough electricity to power a small city. Kumamoto has both in surplus.":
    "半導體晶圓廠每日需消耗<strong>約 3,800 萬公升的超純水</strong>，外加足以供應一座小型城市的電力。熊本兩者皆充裕無虞。",
  "Seoul and Taipei are both just <strong>1 hour 40 minutes</strong> by direct flight from Kumamoto. Busan is 1h 25m. Every major semiconductor hub in Asia is under 3 hours away.":
    "從熊本直飛首爾與台北僅需<strong>1 小時 40 分鐘</strong>，釜山僅需 1 小時 25 分鐘。亞洲所有主要半導體重鎮均在 3 小時航程以內。",
  '<p style="color: var(--color-text-secondary); margin-top: var(--space-2);">Click destinations on the map to see route details.</p>':
    '<p style="color: var(--color-text-secondary); margin-top: var(--space-2);">點選地圖上的目的地可檢視航線詳情。</p>',
  "<strong>4+ trillion yen</strong> from the national government. <strong>480 billion yen</strong> from Kumamoto Prefecture. Every level of government is aligned behind semiconductors.":
    "中央政府投入<strong>超過 4 兆日圓</strong>，熊本縣投入<strong>4,800 億日圓</strong>。從國家到地方，各層級政府全面支持半導體產業。",
  '<p style="margin-top: var(--space-2);">Click tier markers to see commitment details.</p>':
    '<p style="margin-top: var(--space-2);">點選各層級標記可檢視承諾詳情。</p>',
  "TSMC committed <strong>2.16 trillion yen</strong> for two fabs. Sony, SUMCO, Kyocera, Rohm Apollo, Mitsubishi, Tokyo Electron all announced expansions. <strong>Seven major players</strong>, all converging on Kumamoto.":
    "台積電投入<strong>2.16 兆日圓</strong>建設兩座晶圓廠。Sony、SUMCO、京瓷、Rohm Apollo、三菱電機、東京威力科創均已宣布擴廠計劃。<strong>七大核心企業</strong>齊聚熊本。",
  '<p style="margin-top: var(--space-2);">Click company markers to see investment scale.</p>':
    '<p style="margin-top: var(--space-2);">點選企業標記可檢視投資規模。</p>',
  "Kumamoto Science Park anchors a semiconductor corridor backed by <strong>¥4.8 trillion</strong> in government investment. A new airport vision connects the corridor to Asia.":
    "熊本科學園區是半導體走廊的核心，背後有<strong>4.8 兆日圓</strong>的政府投資支撐。全新的大空港構想將走廊與亞洲緊密連結。",
  "METI's Kyushu Semiconductor Human Resources Development Alliance coordinates <strong>five universities</strong> across the region, building a purpose-built talent pipeline.":
    "經產省主導的九州半導體人才育成聯盟協調區域內<strong>五所大學</strong>，打造專屬的人才供給管道。",
  "See the 2030+ completed state: science park expansion, grand airport, road completions, and new stations.":
    "預覽 2030 年以後的完成狀態：科學園區擴張、大空港、道路完工與新設車站。",
  "Three zones in the silicon triangle, each with a distinct role in the semiconductor ecosystem.":
    "矽三角中的三大區域，各自在半導體生態系中扮演不同角色。",
  "Investment properties": "投資物件",

  // =====================================================================
  // Resources (resources.js)
  // =====================================================================

  "Aso Groundwater Basin": "阿蘇地下水盆地",
  "Natural water resources": "天然水資源",
  "Kumamoto sits atop one of Japan's largest groundwater basins, fed by Mount Aso's volcanic soil. This pristine water supply is critical for semiconductor manufacturing, which requires enormous quantities of ultrapure water.":
    "熊本座落於日本最大的地下水盆地之一，由阿蘇火山土壤持續補注。這項優質水源對半導體製造至關重要，因為晶圓廠需要大量超純水。",
  "Cubic meters annual capacity": "年容量（立方公尺）",
  "Natural purity level": "天然純度",
  "Water acquisition cost": "取水成本",
  "Lower than Tokyo rates": "低於東京費率",
  "TSMC ESG evidence": "台積電 ESG 佐證",
  "Official government report on groundwater sustainability and industrial allocation":
    "官方地下水永續性與工業分配報告",
  "TSMC ESG report overview": "台積電 ESG 報告概覽",
  "2024 TSMC ESG report": "2024 年台積電 ESG 報告",
  "Coca-Cola Bottlers Japan": "日本可口可樂瓶裝公司",
  "Kumamoto plant": "熊本工廠",
  "Major beverage manufacturer chose Kumamoto for exceptional water quality and abundance. The plant produces beverages for the entire Kyushu region.":
    "大型飲料製造商因水質卓越且水量充沛而選擇熊本設廠，該工廠供應整個九州地區的飲品。",
  Established: "成立年份",
  Location: "所在地",
  Employees: "員工數",
  Distribution: "配送範圍",
  "Suntory Kyushu Kumamoto Factory": "三得利九州熊本工廠",
  "Premium beverage production": "高端飲品生產",
  "Suntory selected Kashima, Kamimashiki for its pristine groundwater. The facility produces premium beverages requiring the highest water purity standards.":
    "三得利選擇上益城郡嘉島町，正是因為其優質地下水。該設施生產的高端飲品對水質純度有最嚴格的要求。",
  "Product grade": "產品等級",
  "Local water": "在地水源",
  Premium: "頂級",
  "Kyushu Power Grid": "九州電力系統",
  "Power infrastructure": "電力基礎設施",
  "Kyushu Electric provides stable, competitively priced power to the region. The diverse energy mix ensures grid stability critical for semiconductor manufacturing.":
    "九州電力以具競爭力的價格為區域提供穩定電力。多元化的能源組合確保電網穩定性，這對半導體製造至關重要。",
  "Available industrial capacity": "可用工業容量",
  "Grid reliability": "電網可靠度",
  "Industrial rate": "工業電價",
  "Renewable mix": "再生能源占比",
  "Kyushu Electric infrastructure plan": "九州電力基礎設施計劃",
  "Investment roadmap for semiconductor corridor power infrastructure":
    "半導體走廊電力基礎設施投資藍圖",
  "Kyushu leads Japan in energy diversity, providing the stable power semiconductor fabs require.":
    "九州在日本能源多元化方面居領先地位，提供半導體晶圓廠所需的穩定電力。",
  Solar: "太陽能",
  Wind: "風力",
  Nuclear: "核能",
  "Kagoshima 24.7 MW, Fukuoka 22.9 MW, Nagasaki 10 MW":
    "鹿兒島 24.7 MW、福岡 22.9 MW、長崎 10 MW",
  "Miyazaki 65.55 MW, Saga/Nagasaki 27.2 MW, Goto offshore":
    "宮崎 65.55 MW、佐賀/長崎 27.2 MW、五島離岸風電",
  "Genkai (Saga), Sendai (Kagoshima)": "玄海（佐賀）、川內（鹿兒島）",

  // Sewage
  "Kumamoto Semicon public sewage": "熊本半導體特定公共下水道",
  "Specified public sewage district": "特定公共下水道區域",
  "Kumamoto Prefecture designated a Semicon Specified Public Sewage district to support expanded industrial and residential demand from the semiconductor corridor. The master plan covers multiple zones across the Koshi area.":
    "熊本縣劃定半導體特定公共下水道區域，以因應半導體走廊擴張帶來的工業與住宅排水需求。總體計劃涵蓋合志地區的多個區域。",
  "Plan scale": "計劃比例尺",
  "Primary coverage": "主要涵蓋區域",
  "Decision authority": "決策機關",
  "Sewage designation": "下水道類別",
  Specified: "特定",
  Prefecture: "縣",
  "Kumamoto Semicon public sewage master plan": "熊本半導體特定公共下水道總體計劃",
  "Kumamoto Prefecture urban plan sewerage change for Semicon Specified Public Sewage district":
    "熊本縣都市計劃下水道變更——半導體特定公共下水道區域",

  // Silicon Island
  "Japan semiconductor materials dominance": "日本半導體材料主導地位",
  "Global materials and equipment leadership": "全球材料與設備領先地位",
  "Japanese companies hold 8% of global semiconductor production value (¥74.6 trillion market, 2022), but dominate critical materials and equipment: silicon wafers (~60%), photoresist (~70%), encapsulation materials (~80%), and coating equipment (~90%).":
    "日本企業占全球半導體產值的 8%（2022 年市場規模 74.6 兆日圓），但在關鍵材料與設備上居主導地位：矽晶圓（約 60%）、光阻劑（約 70%）、封裝材料（約 80%）、塗佈設備（約 90%）。",
  "Japan production share": "日本生產占比",
  "Materials global share": "材料全球占比",
  "Coating equipment share": "塗佈設備占比",
  "Global market (2022)": "全球市場規模（2022）",
  "Japan semiconductor global market presence (JEITA/WSTS)":
    "日本半導體全球市場地位（JEITA/WSTS）",
  "Japan global semiconductor production share and materials/equipment dominance data":
    "日本全球半導體生產占比與材料/設備主導地位數據",

  // Kyushu Energy
  "Kagoshima Solar Installations": "鹿兒島太陽能設施",
  "Fukuoka Solar Installations": "福岡太陽能設施",
  "Nagasaki Solar Installations": "長崎太陽能設施",
  "Miyazaki Wind Farm": "宮崎風力發電場",
  "Saga-Nagasaki Wind Farm": "佐賀-長崎風力發電場",
  "Goto Offshore Wind": "五島離岸風電",
  "Genkai Nuclear Power Station": "玄海核能發電廠",
  "Sendai Nuclear Power Station": "川內核能發電廠",
  Capacity: "裝置容量",
  "Kyushu solar power capacity": "九州太陽能發電容量",
  "Renewable energy base": "再生能源基礎",
  "Kyushu solar generation capacity has grown steadily since the FIT system launched in 2012. As of September 2024, installed solar capacity across the region reached 12.24 GW, providing the bulk of renewable generation supporting the semiconductor corridor.":
    "自 2012 年 FIT 制度啟動以來，九州太陽能發電容量穩定成長。截至 2024 年 9 月，區域內太陽能裝置容量達 12.24 GW，提供支撐半導體走廊的主要再生能源。",
  "Solar capacity (2024)": "太陽能容量（2024）",
  "Combined Kyushu sites": "九州各站合計",
  "Annual growth": "年成長量",
  "FIT system start": "FIT 制度啟動",
  "Kyushu wind power capacity": "九州風力發電容量",
  "Renewable energy complement": "再生能源互補",
  "Kyushu wind generation complements solar as part of the region's renewable energy base. As of September 2024, installed wind capacity reached 640 MW. The Goto offshore wind project brings deep-water floating turbine technology to the region.":
    "九州風力發電作為區域再生能源基礎的一部分，與太陽能互補。截至 2024 年 9 月，風力裝置容量達 640 MW。五島離岸風電專案將深水浮動式風機技術引入該區域。",
  "Wind capacity (2024)": "風力容量（2024）",
  "Goto floating turbines": "五島浮動式風機",
  "Total renewable (solar+wind)": "再生能源總計（太陽能+風力）",
  "Kyushu nuclear baseload": "九州核能基載電力",
  "Grid stability backbone": "電網穩定骨幹",
  "Genkai and Sendai nuclear power stations provide 5.25 GW of baseload electricity, ensuring the stable 24/7 power supply that semiconductor fabs require. Kyushu Electric's nuclear fleet delivers 99.97% reliability.":
    "玄海與川內核能發電廠提供 5.25 GW 的基載電力，確保半導體晶圓廠所需的 24/7 穩定供電。九州電力核能機組可靠度達 99.97%。",
  "Combined capacity": "總裝置容量",
  "Reliability rate": "可靠度",
  "Baseload operation": "基載運轉",
  "Cost to grid": "併網成本",

  // =====================================================================
  // Government (government.js)
  // =====================================================================

  "Every level of government is aligned behind this corridor.":
    "從中央到地方，各層級政府全面支持此走廊的發展。",
  "Japan National Government": "日本中央政府",
  "Strategic semiconductor policy": "半導體戰略政策",
  "The Japanese government designated semiconductors as critical infrastructure, committing ¥10 billion to support domestic chip production in Kumamoto.":
    "日本政府將半導體列為關鍵基礎設施，投入超過 100 億日圓支持熊本在地晶片生產。",
  "Direct commitment": "直接承諾金額",
  "Policy announced": "政策宣布年份",
  "Infrastructure status": "基礎設施地位",
  "JASM subsidy": "JASM 補助比率",
  Critical: "關鍵",
  "Kumamoto Prefecture": "熊本縣",
  "Regional coordination": "區域統籌",
  "Kumamoto Prefecture allocated additional funds and streamlined permitting for semiconductor-related development across the region.":
    "熊本縣增撥經費並簡化半導體相關開發的審批流程。",
  "Infrastructure budget": "基礎建設預算",
  "Priority projects": "優先專案數",
  "Permit time reduction": "審批時程縮短",
  "Master plan horizon": "總體計劃期程",
  "Kikuyo Town": "菊陽町",
  "Local development plan": "地方發展計劃",
  "Kikuyo approved rezoning for 2,500 housing units and commercial centers to support semiconductor worker families.":
    "菊陽町已核准重新劃區，規劃 2,500 戶住宅與商業中心，以因應半導體產業從業人員家庭的需求。",
  "Housing units": "住宅戶數",
  Infrastructure: "基礎建設",
  "Phase 1 complete": "第一期完成",
  "Population target": "人口成長目標",
  "Ozu Town": "大津町",
  "Industrial expansion": "產業用地擴張",
  "Ozu designated 120 hectares for industrial and logistics use, supporting the semiconductor supply chain.":
    "大津町劃定 120 公頃的產業與物流用地，支持半導體供應鏈發展。",
  "Industrial land": "產業用地",
  "Phase 1": "第一期",
  "Jobs projected": "預估就業人數",
  "New grand airport vision": "新大空港構想",
  'A new 6.8km rail connection will link Aso Kumamoto Airport directly to the JR Hohi Line, with an estimated travel time of 44 minutes from Kumamoto Station. The "New Grand Airport Vision" is a 10-year plan with four strategic pillars to position Kumamoto Airport as a gateway for the semiconductor ecosystem.':
    "一條全新 6.8 公里的軌道連結，將阿蘇熊本機場直接接入 JR 豊肥本線，從熊本站出發預估 44 分鐘可達。「新大空港構想」是一項 10 年計劃，以四大策略支柱將熊本機場定位為半導體生態系的門戶。",
  "New rail link": "新軌道連結",
  "Rail investment": "軌道投資額",
  "Station to airport": "車站至機場",
  "Strategic plan": "策略計劃",
  "Enhance domestic and international route network":
    "強化國內與國際航線網絡",
  "Improve ground transportation and airport access":
    "改善地面交通與機場聯外",
  "Strengthen cargo and logistics capabilities": "強化貨運與物流能力",
  "Develop airport area as regional gateway hub":
    "將機場區域發展為區域門戶樞紐",

  // Government Tiers
  "National Policy": "國家政策",
  "Regional Coordination": "區域統籌",
  Implementation: "執行落實",
  "Local Municipalities": "地方自治體",
  "Direct Investment": "直接投資",
  "Infrastructure Budget": "基礎建設預算",
  "Combined Investment": "合計投資額",
  "Three key local initiatives directly supporting the semiconductor corridor workforce and infrastructure.":
    "三項關鍵地方政策，直接支持半導體走廊的人力與基礎建設需求。",
  "Combined investment": "合計投資額",
  "Key municipalities": "重點自治體",
  "Phase 1 targets": "第一期目標",
  "Housing + jobs": "住宅 + 就業",
  "Residential and commercial": "住宅與商業",
  "Koshi Town": "合志市",
  "TEL R&D hub and high-end rentals": "東京威力科創研發中心與高端租賃",
  "Tokyo Electron (TEL) is building a major R&D hub in Fukuhara, Koshi Town. The ~27,000 sqm facility represents ~¥47 billion in investment, focused on coating, developing, and cleaning equipment R&D. This drives demand for high-income R&D engineers seeking family-size 2-3LDK mid-to-high-end rentals.":
    "東京威力科創（TEL）正在合志市福原建設大型研發中心。約 27,000 平方公尺的設施投資約 470 億日圓，專注於塗佈、顯影與清洗設備的研發。這帶動高收入研發工程師對 2-3LDK 中高端家庭型租賃住宅的需求。",
  "Facility size": "設施面積",
  "Tokyo Electron": "東京威力科創",
  "Rental demand": "租賃需求",

  // =====================================================================
  // Companies (companies.js)
  // =====================================================================

  "JASM (TSMC Japan)": "JASM（台積電日本）",
  "Semiconductor manufacturing": "半導體製造",
  "Joint venture between TSMC, Sony, and Denso. Japan's most advanced semiconductor fab, producing chips for automotive and industrial applications.":
    "由台積電、Sony 及 Denso 合資成立。日本最先進的半導體晶圓廠，生產車用與工業應用晶片。",
  "Total investment": "總投資額",
  "Direct employees": "直接雇用人數",
  "Phase 1 operational": "第一期投產",
  "Process node": "製程節點",
  "JASM press release": "JASM 新聞稿",
  "Official announcement of Phase 2 expansion": "第二期擴建正式公告",
  "Sony Semiconductor": "Sony 半導體",
  "Image sensor production": "影像感測器生產",
  "Sony's flagship image sensor facility supplies Apple, Samsung, and global smartphone manufacturers. Recent expansion doubled production capacity.":
    "Sony 旗艦影像感測器設施供應 Apple、Samsung 及全球智慧型手機製造商。近期擴建使產能倍增。",
  "Expansion investment": "擴建投資額",
  "Global CMOS share": "全球 CMOS 市占率",
  "Expansion complete": "擴建完成",
  "Sony Kumamoto expansion": "Sony 熊本擴建",
  "Facility expansion and hiring announcement": "設施擴建與招募公告",
  "Equipment manufacturing": "設備製造",
  "World's third-largest semiconductor equipment manufacturer. New Kumamoto facility will produce next-generation chip-making tools.":
    "全球第三大半導體設備製造商。熊本新廠將生產新世代晶片製造設備。",
  "Projected jobs": "預估就業人數",
  Opening: "啟用年份",
  "Global equipment rank": "全球設備排名",
  "Tokyo Electron announcement": "東京威力科創公告",
  "New facility press release": "新設施新聞稿",
  "Mitsubishi Electric": "三菱電機",
  "Power semiconductors": "功率半導體",
  "Major expansion of power semiconductor production for electric vehicles and renewable energy systems.":
    "大幅擴張功率半導體產能，以因應電動車與再生能源系統的需求。",
  "New jobs": "新增就業",
  Completion: "完工年份",
  "Capacity increase": "產能增幅",
  "Mitsubishi power semiconductor plan": "三菱電機功率半導體計劃",
  "EV market expansion strategy": "電動車市場擴張策略",
  SUMCO: "SUMCO",
  "Silicon wafer manufacturing": "矽晶圓製造",
  "One of the world's largest silicon wafer manufacturers. SUMCO's Kyushu facilities produce high-purity wafers essential for advanced semiconductor fabrication.":
    "全球最大矽晶圓製造商之一。SUMCO 九州廠區生產先進半導體製程所需的高純度晶圓。",
  "Global wafer share": "全球晶圓市占率",
  "SUMCO Kyushu expansion": "SUMCO 九州擴建",
  "Wafer production capacity expansion plan": "晶圓產能擴增計劃",
  Kyocera: "京瓷",
  "Ceramic packages & components": "陶瓷封裝與元件",
  "Kyocera manufactures ceramic packages and electronic components critical to semiconductor assembly. Their Kyushu operations serve the entire Asia-Pacific market.":
    "京瓷製造半導體封裝所需的關鍵陶瓷封裝與電子元件，九州營運據點服務整個亞太市場。",
  "Core product": "核心產品",
  "Market served": "服務市場",
  "IC packages": "IC 封裝",
  "Asia-Pacific": "亞太地區",
  "Kyocera component expansion": "京瓷元件擴建",
  "Regional manufacturing strategy": "區域製造策略",
  "Rohm Apollo": "Rohm Apollo",
  "Analog & power semiconductors": "類比與功率半導體",
  "Rohm Apollo Semiconductor produces analog ICs and power devices in Kumamoto. Expanding capacity to meet growing EV and industrial automation demand.":
    "Rohm Apollo 半導體在熊本生產類比 IC 與功率元件，正在擴充產能以因應日益成長的電動車與工業自動化需求。",
  "Key technology": "核心技術",
  "SiC power": "碳化矽功率元件",
  "Capacity expansion": "產能擴充",
  "Rohm Apollo SiC expansion": "Rohm Apollo 碳化矽擴建",
  "Silicon carbide power device production plan": "碳化矽功率元件生產計劃",

  // =====================================================================
  // Infrastructure (infrastructure.js)
  // =====================================================================

  // Talent Pipeline
  "The Kyushu Semiconductor Human Resources Development Alliance, led by METI, coordinates talent cultivation across the region to ensure a steady pipeline of semiconductor engineers and technicians.":
    "由經濟產業省（METI）主導的九州半導體人才育成聯盟，協調區域內的人才培育，確保半導體工程師與技術人員的穩定供給。",
  "Ministry of Economy, Trade and Industry": "經濟產業省（METI）",
  "Leads the Kyushu Semiconductor Human Resources Development Alliance":
    "主導九州半導體人才育成聯盟",
  "Strengthen talent cultivation and supply chain stability":
    "強化人才培育與供應鏈穩定性",
  Kyutech: "九州工業大學",
  "Kyushu Institute of Technology": "九州工業大學",
  "Established a cross-departmental semiconductor human resources center to train the next generation of chip engineers.":
    "成立跨系所半導體人力資源中心，培育次世代晶片工程師。",
  "Annual graduates": "年畢業人數",
  "Program scope": "計劃範疇",
  "Cross-departmental": "跨系所",
  "Core strength": "核心優勢",
  Research: "研究",
  "Kyushu University": "九州大學",
  "Established an adult semiconductor retraining center for working professionals seeking to transition into the chip industry.":
    "設立在職人士半導體轉職培訓中心，協助專業人員轉入晶片產業。",
  "Annual intake": "年招收人數",
  "Target audience": "目標對象",
  "Working adults": "在職成人",
  "University rank": "大學排名",
  "Top 5 in Japan": "日本前五",
  "Oita University": "大分大學",
  "Established a semiconductor core talent retraining center focused on upskilling working professionals through industry-led partnerships.":
    "設立半導體核心人才再訓練中心，透過產業主導的合作模式提升在職專業人員技能。",
  "Program focus": "計劃重點",
  "Core talent development": "核心人才養成",
  "Working professionals": "在職專業人員",
  "Partnership model": "合作模式",
  "Industry-led": "產業主導",
  "Kumamoto University": "熊本大學",
  "Partnered with JASM (TSMC) to establish a semiconductor research center as a direct industry-academia collaboration.":
    "與 JASM（台積電）合作設立半導體研究中心，實現產學直接合作。",
  "Industry partner": "產業合作夥伴",
  "JASM (TSMC)": "JASM（台積電）",
  Researchers: "研究員數",
  Model: "模式",
  "Industry-academia": "產學合作",
  "Prefectural University of Kumamoto": "熊本縣立大學",
  "Since 2023, all first-year science and engineering students are required to take semiconductor introductory courses taught by industry experts.":
    "自 2023 年起，所有理工學院一年級學生必修由業界專家授課的半導體入門課程。",
  "Mandate start": "必修開始",
  Scope: "適用範圍",
  "All first-year students": "所有一年級學生",
  Instructors: "授課師資",
  "Industry experts": "業界專家",

  // Employment Data
  "JASM and major semiconductor employers are reshaping Kumamoto's wage landscape, offering salaries significantly above local and national averages to attract talent.":
    "JASM 與主要半導體雇主正在重塑熊本的薪資結構，以顯著高於在地與全國平均的薪資水準延攬人才。",
  "University graduate salary": "大學畢業生薪資",
  "JASM offers ¥280,000/month for university graduates, ¥320,000/month for master's degrees, and ¥360,000/month for doctorates. Significantly higher than the local Kumamoto average for graduates (~¥201,000/month).":
    "JASM 大學畢業生月薪 28 萬日圓、碩士 32 萬日圓、博士 36 萬日圓，顯著高於熊本在地畢業生平均（約 20.1 萬日圓/月）。",
  "University graduate": "大學畢業",
  "Master's degree": "碩士",
  Doctorate: "博士",
  "Kumamoto average": "熊本平均",
  "METI semiconductor workforce report": "經產省半導體人力報告",
  "JASM's monthly salary is ¥50,000 or more higher than the national average.":
    "JASM 月薪較全國平均高出 5 萬日圓以上。",
  "Tokyo Electron (TEL)": "東京威力科創（TEL）",
  "Salary increase for new recruits": "新進員工薪資調幅",
  "TEL broke the ¥300,000 barrier for new recruits to stay competitive with TSMC and ASML. Effective April 2024, monthly salary for university graduates reached ¥304,800 (increase of ¥85,500/month).":
    "TEL 為與台積電和 ASML 競爭人才，將新進員工起薪突破 30 萬日圓大關。自 2024 年 4 月起，大學畢業生月薪達 304,800 日圓（月增 85,500 日圓）。",
  "New graduate salary": "新進畢業生薪資",
  "Monthly increase": "月薪增幅",
  "Salary increase": "薪資調幅",
  "Effective date": "生效日期",
  "TEL quarterly earnings report": "TEL 季度財報",

  // Science Park
  "Kumamoto Science Park Corridor": "熊本科學園區走廊",
  "Government development zone": "政府開發區",
  "The Kumamoto Prefectural Government has designated this area as a special semiconductor development zone, offering tax incentives, streamlined permitting, and infrastructure investments totaling ¥4.8 trillion.":
    "熊本縣政府將此區域劃定為半導體特別開發區，提供租稅優惠、簡化審批流程及總計 4.8 兆日圓的基礎建設投資。",
  "Government investment": "政府投資額",
  "Completion target": "完成目標年",
  "Projected new jobs": "預估新增就業",
  "Major facilities planned": "計劃重大設施數",
  "Kumamoto Science Park master plan": "熊本科學園區總體計劃",
  "Official development roadmap and zoning documentation":
    "官方開發藍圖與分區規劃文件",

  // Zone Plans
  "The prefectural government designated a special semiconductor zone within the Science Park boundary. This zone provides tax incentives, streamlined permitting, and dedicated infrastructure for semiconductor-related industries.":
    "縣政府在科學園區範圍內劃定半導體特別區域，提供租稅優惠、簡化審批流程及專屬基礎設施。",
  "Designated area": "劃定面積",
  "Public investment": "公共投資",
  "Zoning enacted": "分區生效",
  "Incentive programs": "優惠方案數",
  "Semiconductor cluster area": "半導體聚落區域",
  "Kochi municipal / Kikuyo Town industrial park":
    "合志市營 / 菊陽町工業園區",
  "West of JASM": "JASM 西側",
  "The broader Semicon Techno Park zone straddling the Fukuhara–Haramizu boundary, west of JASM along Route 30 and the Kikuyo bypass. Includes Semicon Techno Park, Koshi Industrial Park, and Haramizu Industrial Park.":
    "位於 JASM 西側、橫跨福原至原水一帶的半導體科技園區，沿 30 號線與菊陽繞道分布，涵蓋半導體科技園區、合志工業園區及原水工業園區。",
  "Industrial parks": "工業園區數",
  "Phase 1 open": "第一期啟用",
  "Kumamoto Prefecture industrial park": "熊本縣營工業園區",
  "Kikuchi City": "菊池市",
  "North of JASM": "JASM 北側",
  "Prefecture-operated industrial park north of JASM, part of the distributed Science Park framework.":
    "位於 JASM 北側的縣營工業園區，為分散式科學園區架構的一環。",
  "Site area": "基地面積",
  "Kyokushikawabe zone": "旭志川邊區域",
  "Northeast of JASM": "JASM 東北側",
  "Kumamoto North Industrial Park northeast of JASM, forming part of the wider semiconductor corridor expansion across Kumamoto Prefecture.":
    "位於 JASM 東北側的熊本北部工業園區，為熊本縣半導體走廊擴張的一環。",
  "Ozu town-operated industrial zone": "大津町營工業區",
  "East of JASM": "JASM 東側",
  "Industrial corridor east of JASM including Honda Kumamoto Factory and Ozu South Industrial Park.":
    "位於 JASM 東側的產業走廊，涵蓋本田熊本工廠及大津南部工業園區。",
  Jobs: "就業人數",
  "Kumamoto airport area": "熊本機場區域",
  "Southeast of JASM": "JASM 東南側",
  "Aso Kumamoto Airport and surrounding development zone, southeast of JASM.":
    "阿蘇熊本機場及周邊開發區域，位於 JASM 東南側。",
  "Kikuyo town rezoned development district": "菊陽町重劃開發區",
  "Southwest of JASM": "JASM 西南側",
  "Planned land readjustment and rezoning area in the Kubota–Tsukure district of Kikuyo, southwest of JASM.":
    "位於 JASM 西南側菊陽町久保田—津久禮地區的計劃性土地區劃整理與重新劃區區域。",
  "Rezoned area": "重劃面積",
  "Municipal budget": "自治體預算",
  Target: "目標年",
  "Sony Semiconductor Kumamoto": "Sony 半導體熊本",

  "Kikuyo Town has approved rezoning for mixed-use development adjacent to the Science Park. New housing, retail, medical, and support services planned for semiconductor workers and their families.":
    "菊陽町已核准科學園區毗鄰地區的複合式開發重劃，規劃住宅、零售、醫療及支援服務設施，以服務半導體產業從業人員及其家庭。",
  "New housing units": "新建住宅戶數",
  "Target completion": "預計完成年",

  "Ozu Town is developing new industrial parcels and logistics facilities to support the semiconductor supply chain. 120 hectares designated for industrial and logistics use.":
    "大津町正在開發新的產業用地與物流設施，以支持半導體供應鏈。120 公頃劃定為產業與物流用途。",
  "Industrial parcels": "產業用地",
  "Infrastructure spend": "基礎建設支出",
  "Phase 1 complete": "第一期完成",
  "Logistics jobs": "物流就業人數",

  // Grand Airport
  "Higo-Ozu airport access road": "肥後大津機場聯絡道路",
  "New access road connecting Kumamoto Airport to the Ozu development corridor and the semiconductor park.":
    "連接熊本機場與大津開發走廊及半導體園區的新聯絡道路。",
  "Under planning": "規劃中",
  "Kumamoto airport monorail": "熊本機場單軌",
  "Aso Kumamoto Airport": "阿蘇熊本機場",
  "Kumamoto's primary airport handling 3.5 million passengers annually. The proposed rail link will cut travel time to the semiconductor corridor to under 15 minutes.":
    "熊本主要機場，年旅客量 350 萬人次。規劃中的軌道連結將把到半導體走廊的交通時間縮短至 15 分鐘以內。",
  "Annual passengers": "年旅客量",
  "Proposed rail time to JASM": "到 JASM 的規劃軌道時間",
  "Higo-Ozu Station": "肥後大津站",
  "Key JR Hohi Line junction where the proposed airport access railway will terminate, connecting air and rail transit for the semiconductor corridor.":
    "JR 豊肥本線的重要節點，規劃中的機場聯絡鐵路將在此站匯入，串聯航空與鐵道運輸。",
  "Railway line": "鐵路路線",
  "To Kumamoto Station": "至熊本站",
  "Semiconductor Tech Park": "半導體科技園區",
  "A dedicated technology park for semiconductor-related R&D and manufacturing support facilities, anchoring the northern end of the corridor.":
    "專為半導體相關研發與製造支援設施設置的科技園區，為走廊北端的核心據點。",
  "Planned area": "計劃面積",
  "Target completion": "目標完成年",
  "New Kikuyo Station": "菊陽新站",
  "Planned new station on the JR Hohi Line serving the semiconductor corridor workforce. Will reduce commute times for thousands of employees.":
    "JR 豊肥本線上的新設車站，服務半導體走廊從業人員，將大幅縮短數千名員工的通勤時間。",
  "Planned opening": "計劃啟用",
  "Projected daily riders": "預估每日旅客量",
  "Techno Research Park": "科技研究園區",
  "Established research park housing semiconductor testing and advanced materials labs. Benefits directly from improved airport and rail connectivity.":
    "已設立的研究園區，設有半導體測試與先進材料實驗室，直接受益於機場與鐵路交通改善。",
  "Research tenants": "研究進駐單位",
  "Previously announced route": "先前公布路線",
  "The initial proposed alignment for the airport access railway, following a direct path from Aso Kumamoto Airport to Higo-Ozu Station.":
    "機場聯絡鐵路最初規劃的路線方案，從阿蘇熊本機場直通肥後大津站。",
  "Estimated length": "預估長度",
  "Alignment type": "路線型態",
  Direct: "直線",
  "Newly announced route": "新公布路線",
  "The revised route announced in 2024, sweeping east to better serve the Techno Research Park area before connecting to Higo-Ozu Station.":
    "2024 年公布的修訂路線，向東繞行以服務科技研究園區一帶，再接入肥後大津站。",
  Curved: "彎道型",
  "JR Hohi Line": "JR 豊肥本線",
  "The existing east-west JR railway connecting Kumamoto City to Oita Prefecture. The airport access railway will join this line at Higo-Ozu Station.":
    "連接熊本市與大分縣的既有東西向 JR 鐵路。機場聯絡鐵路將在肥後大津站接入此路線。",
  "Total length": "總長度",
  "Airport connection road": "機場連絡道路",
  "New high-standard road connecting Kumamoto IC directly east to Aso Kumamoto Airport. Reduces airport access time from the city center to approximately 20 minutes.":
    "連接熊本交流道直通東側阿蘇熊本機場的新建高標準道路，將市中心到機場的交通時間縮短至約 20 分鐘。",
  Length: "長度",
  "Airport target": "機場連結目標",
  "20-min access": "20 分鐘可達",
  "Kumamoto ring road connector": "熊本環狀道路連接線",
  "Planned high-standard road running from Kumamoto North Junction westward to Ozu interchange. Part of the Kumamoto ring road network linking the semiconductor corridor to the expressway system.":
    "從熊本北交匯處向西延伸至大津交流道的高標準道路。為熊本環狀道路網絡的一部分，將半導體走廊與高速公路系統串聯。",
  "Ring road": "環狀道路",
  Concept: "概念",

  // Railway Stations
  "Kumamoto station": "熊本站",
  "Major terminal station and starting point of the JR Hohi Main Line. Principal rail hub for Kumamoto Prefecture with shinkansen connections.":
    "JR 豊肥本線的起點大站，也是熊本縣主要鐵路樞紐，可轉乘新幹線。",
  "Daily riders": "每日旅客量",
  "Sanrigi station": "三里木站",
  "Station on the JR Hohi Main Line in the Kikuyo area, serving the growing residential district near the semiconductor corridor.":
    "JR 豊肥本線菊陽地區車站，服務半導體走廊周邊快速成長的住宅區。",
  "Proposed new station": "新設車站規劃",
  "Planning-stage concept for a new station on the JR Hohi Main Line between Sanrigi and Haramizu, intended to serve the semiconductor corridor workforce directly.":
    "JR 豊肥本線三里木至原水之間的新站規劃概念，旨在直接服務半導體走廊從業人員。",
  Status: "狀態",
  "Haramizu station": "原水站",
  "Existing station on the JR Hohi Main Line east of the semiconductor corridor. Serves as a key access point for the Kikuyo residential area.":
    "JR 豊肥本線上位於半導體走廊東側的既有車站，為菊陽住宅區的重要出入節點。",
  "Higo-Ozu station": "肥後大津站",
  "Key JR Hohi Main Line junction where the proposed airport access railway will terminate, connecting air and rail transit for the semiconductor corridor.":
    "JR 豊肥本線的重要節點車站，規劃中的機場聯絡鐵路將在此站匯入，串聯航空與鐵道運輸服務半導體走廊。",

  // 10-20 min concept
  "Northern connecting road": "北側連絡道路",
  "Eastern connecting road": "東側連絡道路",
  "Southern connecting road": "南側連絡道路",
  "Kumamoto City Hall": "熊本市役所",
  "Kumamoto Prefectural Office": "熊本縣廳",
  "Kumamoto IC": "熊本交流道",
  "Mashiki area": "益城地區",

  // Road extensions
  "Road type": "道路類型",
  New: "新建",

  // =====================================================================
  // Properties (properties.js)
  // =====================================================================

  "Kikuyo Development Zone": "菊陽開發區",
  "Residential & commercial": "住宅與商業",
  "Kikuyo Town has approved rezoning for mixed-use development adjacent to the Science Park. New housing, retail, and support services for semiconductor workers.":
    "菊陽町已核准科學園區毗鄰地區的複合式開發重劃，規劃住宅、零售及支援服務設施。",
  "Housing units planned": "計劃住宅戶數",
  "To JASM": "到 JASM",
  "Kikuyo Town development plan": "菊陽町發展計劃",
  "Official rezoning and infrastructure roadmap": "官方重劃與基礎建設藍圖",
  Housing: "住宅",
  "Retail hub": "零售中心",
  School: "學校",
  Medical: "醫療",

  "Ozu Industrial Expansion": "大津產業擴張",
  "Industrial & logistics": "產業與物流",
  "Ozu Town is developing new industrial parcels and logistics facilities to support the semiconductor supply chain.":
    "大津町正在開發新的產業用地與物流設施，以支持半導體供應鏈。",
  Available: "可用年份",
  "Ozu industrial zone plan": "大津工業區計劃",
  "Industrial development documentation": "產業開發文件",
  Logistics: "物流",
  Warehouse: "倉儲",
  "Supplier park": "供應商園區",

  "Koshi R&D Corridor": "合志研發走廊",
  "R&D and innovation": "研發與創新",
  "Koshi City is emerging as an R&D hub with Tokyo Electron's new facility and supporting tech infrastructure for the semiconductor corridor.":
    "隨著東京威力科創新設施與配套科技基礎建設的到位，合志市正崛起為研發中心。",
  "TEL investment": "TEL 投資額",
  "R&D facility": "研發設施",
  "R&D jobs": "研發就業人數",
  "Koshi City development plan": "合志市發展計劃",
  "R&D corridor expansion documentation": "研發走廊擴張文件",

  // Corridor Zones
  "Factory core / new urban core": "製造核心 / 新都心",
  "Gateway / office and logistics support": "交通門戶 / 辦公與物流支援",
  "R&D / tools and process innovation": "研發 / 設備與製程創新",

  // GKTK Fund
  "GKTK Fund": "GKTK 基金",
  "Greater Kumamoto Technology Corridor Fund": "大熊本科技走廊基金",
  "Target AUM": "目標管理資產",
  "Real estate investment in the semiconductor corridor":
    "半導體走廊不動產投資",
  "Fund size": "基金規模",
  Vintage: "基金年份",
  "Hold period": "持有期間",
  "Target IRR": "目標 IRR",

  // Properties
  "Chateau Life Ozu 1": "Chateau Life 大津 1 號",
  "Chateau Life Ozu 2": "Chateau Life 大津 2 號",
  "Chateau Life Kikuyo 1": "Chateau Life 菊陽 1 號",
  "Chateau Life Haramizu 1": "Chateau Life 原水 1 號",
  "Chateau Life Haramizu 2": "Chateau Life 原水 2 號",
  "New construction (BTR)": "新建物件（出租型）",
  "Build to rent": "新建出租型",
  "Renovation opportunity": "翻修投資機會",
  "Buy-renovate-rent/sell": "買入-翻修-出租/出售",
  "Land development": "土地開發",
  "Land acquisition": "土地取得",
  "Kikuyo station": "菊陽站",

  // Property Truth Engine
  "Detached house (single-family)": "獨棟住宅（一戶建）",
  "Land (pre-sale / off-plan)": "土地（預售 / 預定案）",
  "Expat family standard spec": "外派家庭標準規格",
  "Large living/dining": "大客餐廳",
  "Dishwasher, floor heating, high insulation": "洗碗機、地暖、高隔熱",
  "Ample storage, EV charging prep": "充足收納、電動車充電預留",
  "Standardized facade and modular floor plans":
    "標準化外觀與模組化平面配置",
  "Compressed construction period and cost volatility":
    "壓縮工期、降低成本波動",
  "Three-factor balance: school district + living amenities + commute":
    "三要素平衡：學區 + 生活機能 + 通勤",
  "Fragmented land": "土地零散",
  "Site preparation costs": "整地費用",
  "Drainage/foundation improvement costs uncertain":
    "排水/地基改良費用不確定",
  "Convert to expat standard": "翻修至外派標準",
  "Insulation and window upgrades": "隔熱與窗戶升級",
  "Traffic flow reorganization": "動線重整",
  "Kitchen and bath quality improvement": "廚衛品質提升",
  "Storage optimization": "收納優化",
  "Lighting and moisture control": "照明與防潮處理",
  "Advantages: property tax and acquisition cost controllable, fast turnaround, can replicate across multiple small properties":
    "優勢：持有稅與取得成本可控、周轉快、可複製至多筆小型物件",
  "Hidden construction issues (leaks, termites, foundation)":
    "隱蔽工程風險（漏水、白蟻、地基）",
  "Resale market depth uncertainty": "再出售市場深度不確定",
  "Renovation cost overrun risk": "翻修成本超支風險",
  "Three-zone development concept": "三區開發概念",
  "Vibrancy zone: station-front retail, F&B, international-friendly services":
    "活力區：站前零售、餐飲、國際化服務",
  "Knowledge cluster: R&D offices, co-working, university satellite campus":
    "知識聚落：研發辦公、共享空間、大學衛星校區",
  "Live-work zone: mid-high density condos, serviced apartments for engineers":
    "職住一體區：中高密度集合住宅、工程師服務式公寓",
  "Facility introduction: residential, apartments, hotels, university campus":
    "導入機能：住宅、公寓、飯店、大學校區",
  "Long-term city-level project, not single housing development. JR Kyushu new station between Mitsuriki and Haramizu creates transport anchor.":
    "長期城市級專案，而非單一住宅開發案。JR 九州在三里木至原水之間新設車站，形成交通錨點。",
  "Land readjustment timeline uncertainty": "土地區劃整理時程不確定",
  "Zoning finalization dependent on municipal process":
    "分區定案取決於自治體流程",
  "Higher upfront capital requirement than renovation path":
    "前期資本需求高於翻修路徑",
  "BTR (build to rent)": "BTR（新建出租型）",
  "Land acquisition and hold/develop": "土地取得-持有/開發",

  // Property Future Outlook
  "Area development plans affecting Ozu 1": "影響大津 1 號的區域開發計劃",
  "Area development plans affecting Ozu 2": "影響大津 2 號的區域開發計劃",
  "Area development plans affecting Kikuyo 1": "影響菊陽 1 號的區域開發計劃",
  "Ozu industrial expansion": "大津產業擴張",
  "120ha new logistics and supply chain facilities by 2027":
    "2027 年前新增 120 公頃物流與供應鏈設施",
  "Naka-Kyushu Cross Road": "中九州橫斷道路",
  "Ozu-Kumamoto Road segment under construction, expressway link through corridor":
    "大津-熊本路段施工中，高速公路貫穿走廊",
  "Science park expansion": "科學園區擴張",
  "Government commitment extends development corridor southward":
    "政府承諾將開發走廊向南延伸",
  "Kikuyo Station expansion": "菊陽站擴建",
  "New train station with direct Kumamoto City line, completion 2026":
    "新設車站直通熊本市，2026 年完工",
  "International school": "國際學校",
  "English-language school for TSMC engineer families, +8% rental premium":
    "為台積電工程師家庭設立的英語學校，租金溢價 +8%",
  "JASM Phase 2": "JASM 第二期",
  "Second fab adding 3,000 employees, +25% rental demand by 2027":
    "第二座晶圓廠新增 3,000 名員工，2027 年前租賃需求增加 25%",
  "Haramizu 70ha development": "原水 70 公頃開發案",
  "Adjacent new urban core drives area transformation":
    "毗鄰的新都心推動區域轉型",
  "70-hectare new urban core with national development partners":
    "70 公頃新都心，與全國性開發商合作",
  "New JR station": "JR 新站",
  "JR Kyushu confirmed new station between Mitsuriki and Haramizu, direct rail to Kumamoto City":
    "JR 九州確認在三里木至原水之間設新站，直通熊本市",
  "Mitsui Fudosan partnership": "三井不動產合作",
  "Japan largest developer selected for long-term vision implementation":
    "日本最大開發商獲選執行長期願景",
  "Foreign consultation counter": "外國人諮詢窗口",
  "Kikuyo Town established bilingual support (Chinese/English) for international residents":
    "菊陽町設立中英雙語國際居民服務窗口",
  "Science park adjacency": "科學園區毗鄰",
  "Direct proximity to semiconductor cluster drives sustained demand":
    "緊鄰半導體聚落帶動持續需求",

  // Property Financial
  "AI rent assessment (4LDK/89sqm)": "AI 租金評估（4LDK/89 平方公尺）",
  "Assessed rent ¥160,000/month from comparable properties":
    "依可比物件評估月租金 16 萬日圓",
  "AI rent assessment (4LDK/95sqm)": "AI 租金評估（4LDK/95 平方公尺）",
  "Assessed rent from comparable properties": "依可比物件評估租金",
  "AI rent assessment (4LDK/115sqm)": "AI 租金評估（4LDK/115 平方公尺）",
  "Real estate investment analysis": "不動產投資分析",
  "Comprehensive land value and development ROI projections":
    "土地價值與開發投資報酬率綜合預估",

  // =====================================================================
  // Data Layers (data-layers.js)
  // =====================================================================

  "Seoul Incheon": "首爾仁川",
  "South Korea": "南韓",
  Korea: "韓國",
  "Samsung memory division HQ link": "三星記憶體事業部總部連結",
  "Direct service to Seoul's primary international airport. Year-round daily flights.":
    "直飛首爾主要國際機場，全年每日運航。",
  Samsung: "三星",
  "Memory Division HQ": "記憶體事業部總部",
  "Busan Gimhae": "釜山金海",
  "Korea's second-largest city": "韓國第二大城市",
  "Daily direct service on Boeing 737.": "波音 737 每日直飛航班。",
  "Taiwan Taoyuan": "台灣桃園",
  Taiwan: "台灣",
  "TSMC headquarters connection": "台積電總部連結",
  "Direct service to Taiwan's main international gateway.":
    "直飛台灣主要國際門戶。",
  TSMC: "台積電",
  "Global Headquarters": "全球總部",
  Kaohsiung: "高雄",
  "Southern Taiwan industrial access": "南台灣產業樞紐連結",
  "Direct service to Taiwan's second-largest city and southern semiconductor hub.":
    "直飛台灣第二大城市暨南部半導體重鎮。",

  // Infrastructure Roads
  "Ozu-Kumamoto Road": "大津-熊本道路",
  "Under Construction": "施工中",
  TBD: "待定",
  "Not disclosed": "未公開",
  Adjacent: "毗鄰",
  "Longest segment of the Naka-Kyushu Cross Road, connecting Koshi to Ozu-West IC in two sections (9.1km Koshi to Kumamoto, started 2020; 4.7km Ozu-West to Koshi, started 2022). Runs directly north of the semiconductor corridor.":
    "中九州橫斷道路最長路段，分兩段連接合志至大津西交流道（合志至熊本 9.1 公里，2020 年動工；大津西至合志 4.7 公里，2022 年動工），直接位於半導體走廊北側。",
  "Ozu Road": "大津道路",
  "Under Survey": "調查中",
  "Eastern segment connecting Ozu-West IC to the existing Ozu IC interchange. Project commenced in 2024 and is currently under survey. Completes the expressway link to the Ozu area east of the semiconductor corridor.":
    "東段連接大津西交流道與既有大津交流道。2024 年開始調查。完成半導體走廊東側大津地區的高速公路連結。",
  "Kumamoto Ring Road": "熊本環狀道路",
  "Western link": "西側連結",
  "Western extension connecting the Ueki IC area to the Kumamoto-Kita junction. Project commenced in 2025 and is currently under survey. Will provide expressway access from western Kumamoto into the corridor.":
    "西段延伸連接植木交流道至熊本北交匯處。2025 年開始調查。將提供西熊本進入走廊的高速公路通道。",

  // Infrastructure Station
  "Kikuyo Station": "菊陽站",
  "New rail connection": "新軌道連結",
  "New JR Hohi Line station providing direct rail access from Kumamoto City to the Science Park corridor. Reduces commute time for semiconductor workers.":
    "JR 豊肥本線新設車站，提供從熊本市到科學園區走廊的直達鐵路，縮短半導體從業人員通勤時間。",
  "To Kumamoto City": "到熊本市",
  "Train frequency": "列車班距",
  "Daily passengers est.": "預估每日旅客量",
  "Rail option": "軌道選項",

  // Haramizu Station
  "Haramizu Station Area": "原水站周邊區域",
  "New development hub": "新開發中心",
  "Under Development": "開發中",
  "Haramizu Station area is being developed as a new urban core with 70ha of mixed-use land. Three development zones planned. Mitsui Fudosan and JR Kyushu selected as development partners.":
    "原水站周邊正被開發為 70 公頃複合式用地的新都心。規劃三大開發區域。三井不動產與 JR 九州獲選為開發夥伴。",
  "Development area": "開發面積",
  "Mixed-use plan": "複合用途計劃",
  "Development partners": "開發夥伴",
  "Phase 1 target": "第一期目標",
  Vibrancy: "活力區",
  "Station-front retail, F&B, international-friendly services":
    "站前零售、餐飲、國際化服務",
  "Knowledge cluster": "知識聚落",
  "R&D offices, co-working, university satellite":
    "研發辦公、共享空間、大學衛星校區",
  "Live-Work": "職住一體",
  "Mid-high density condos, serviced apartments for engineers":
    "中高密度集合住宅、工程師服務式公寓",
  "New urban core": "新都心",

  // Data Layer Names & Descriptions
  "Corporate sites": "企業據點",
  "Major semiconductor manufacturers operating within the Kumamoto corridor.":
    "在熊本走廊內營運的主要半導體製造商。",
  "Major fabs": "主要晶圓廠",
  "Direct employees": "直接雇用人數",
  "Combined investment": "合計投資額",
  "Investment properties in the semiconductor corridor development zone.":
    "半導體走廊開發區內的投資物件。",
  "Avg appreciation": "平均增值率",
  "Rental yield": "租金收益率",
  Occupancy: "出租率",
  "Traffic flow": "交通流量",
  "Real-time and historical traffic patterns across the Kumamoto semiconductor corridor.":
    "熊本半導體走廊的即時與歷史交通動態。",
  "Route 57 Junction": "57 號線交匯處",
  Moderate: "中度",
  "JASM Access Road": "JASM 聯絡道路",
  "Heavy (peak)": "尖峰壅塞",
  "Kikuyo Bypass": "菊陽繞道",
  Light: "輕度",
  "Route 57 Main": "57 號線幹線",
  "Local roads north": "北部地方道路",
  "Local roads south": "南部地方道路",
  "Increase since 2023": "2023 年以來增幅",
  "Work commuters": "通勤人數占比",
  "Road expansion program": "道路擴建計劃",
  Active: "進行中",
  "Rail commute": "鐵路通勤",
  "JR Kyushu rail network serving the semiconductor corridor workforce.":
    "JR 九州鐵路網絡服務半導體走廊從業人員。",
  "Major Hub": "主要樞紐",
  "New Station": "新站",
  "Kikuyo Station (Planned)": "菊陽站（規劃中）",
  "Suizenji Station": "水前寺站",
  "Transfer Hub": "轉乘站",
  "JR Kagoshima Line": "JR 鹿兒島本線",
  "Planned Extension to Science Park": "科學園區延伸規劃",
  "Daily commuters": "每日通勤人次",
  "Avg. to JASM": "到 JASM 平均",
  "Electricity usage": "用電量",
  "Regional power consumption and capacity for industrial operations.":
    "區域工業營運用電量與容量。",
  "Science Park Grid": "科學園區電網",
  "Sony Substation": "Sony 變電站",
  "Tokyo Electron Hub": "東京威力科創樞紐",
  "Grid capacity": "電網容量",
  Uptime: "運轉率",
  "Semiconductor industry employment statistics and hiring trends.":
    "半導體產業就業統計與招募趨勢。",
  "Direct jobs": "直接就業人數",
  "YoY growth": "年成長率",
  "Avg. salary": "平均薪資",
  "Infrastructure plan": "基礎建設計劃",
  "Planned and in-progress infrastructure development projects.":
    "已規劃與進行中的基礎建設專案。",
  "New water treatment": "新水處理設施",
  "Logistics hub": "物流中心",
  "Data center complex": "資料中心園區",
  Planned: "規劃中",
  "Major projects": "重大專案數",
  "Real estate": "不動產",
  "Property market trends and investment activity in the corridor.":
    "走廊內的不動產市場趨勢與投資活動。",
  "Kikuyo Residential Zone": "菊陽住宅區",
  "Ozu Development Area": "大津開發區",
  "Mashiki Township": "益城町",
  "Avg. appreciation": "平均增值率",
  "Occupancy rate": "出租率",
  "Risky area": "風險區域",
  "Flood zones, seismic risk areas, and natural hazard information.":
    "淹水區域、地震風險區域與天然災害資訊。",
  "Shirakawa Flood Zone": "白川淹水區域",
  "Volcanic Proximity": "火山鄰近區",
  "Liquefaction Zone": "液化潛勢區",
  Flood: "洪水",
  Volcanic: "火山",
  Seismic: "地震",
  "Levee upgrade 2025": "堤防升級 2025",
  "30km from Aso caldera": "距阿蘇火山口 30 公里",
  "Building code compliance": "建築法規合規",
  "Overall risk rating": "整體風險評級",
  Low: "低",
  "Last major event": "最近重大事件",
  "Mitigation investment": "防災投資",
  "Science park clusters": "科學園區聚落",
  "Kumamoto Science Park development zones with district-level plans for Kikuyo and Ozu municipalities.":
    "熊本科學園區開發區域，含菊陽町與大津町各區計劃。",
  "Zone plans": "區域計劃",
  "Education & talent pipeline": "教育與人才管道",
  "Universities, training centers, and employment data supporting the semiconductor corridor workforce.":
    "支持半導體走廊人力的大學、培訓中心與就業數據。",
  "Composite 2030+ vision including science park expansion, road infrastructure, and development zones.":
    "2030+ 複合願景，涵蓋科學園區擴張、道路基礎設施與開發區域。",
  "Vision horizon": "願景期程",
  "Investment opportunity zones": "投資機會區域",
  "Three zones in the silicon triangle, each with a distinct role in the semiconductor ecosystem.":
    "矽三角中的三大區域，各自在半導體生態系中扮演不同角色。",
  "Base map": "底圖",
  "Standard geographic reference markers and points of interest.":
    "標準地理參考標記與景點。",
  "Kumamoto City Center": "熊本市中心",
  City: "城市",
  "Kumamoto Airport": "熊本機場",
  Airport: "機場",
  "Mount Aso": "阿蘇山",
  Landmark: "地標",
  "City population": "城市人口",
  "Prefecture pop.": "縣人口",
  "Japan metro rank": "日本都市排名",

  // Demand Projections
  "JASM Phase 1 operational": "JASM 第一期投產",
  "Sony expansion + Tokyo Electron opening":
    "Sony 擴建 + 東京威力科創啟用",
  "JASM Phase 2 construction workforce": "JASM 第二期建設人力",
  "Full corridor operational": "走廊全面運作",
  "Stabilization at capacity": "產能穩定",
  "Current housing stock serves 65% of projected demand. New construction permits lag behind workforce arrival by 12-18 months, creating sustained rental pressure through 2027.":
    "現有住宅存量僅供應預估需求的 65%。新建許可落後人力到位 12-18 個月，造成至 2027 年的持續租賃壓力。",
  "Semiconductor shift work creates year-round demand with no seasonal dip. April hiring cycles cause 15-20% rental inquiry spikes.":
    "半導體輪班制帶來全年無淡季的需求。四月招募週期造成 15-20% 的租賃詢問量增幅。",

  // =====================================================================
  // Evidence Groups (evidence.js)
  // =====================================================================

  "Energy infrastructure": "能源基礎設施",
  "Solar capacity (2024.9)": "太陽能容量（2024.9）",
  "Solar capacity (2012)": "太陽能容量（2012）",
  "Growth from FY2023": "較 FY2023 成長",
  "Wind capacity (2024.9)": "風力容量（2024.9）",
  "Wind capacity (2012)": "風力容量（2012）",
  "Total renewable growth since 2012": "自 2012 年再生能源總成長",
  "Kyushu nuclear (Sendai)": "九州核能（川內）",
  "Sendai Nuclear Power Plant provides baseload electricity for the region, ensuring stable power supply for high-demand semiconductor manufacturing.":
    "川內核能發電廠為區域提供基載電力，確保高用電需求的半導體製造穩定供電。",
  "Generation capacity": "發電容量",
  "Kumamoto Semicon public sewage plan": "熊本半導體特定公共下水道計劃",
  "Kumamoto Prefecture urban plan sewerage change designating the Kumamoto Semicon Specified Public Sewage district. The 1:12,000 master plan map covers multiple zones across the Koshi and semiconductor corridor area to support expanded industrial and residential demand.":
    "熊本縣都市計劃下水道變更，劃定半導體特定公共下水道區域。1:12,000 總體計劃圖涵蓋合志及半導體走廊區域的多個分區，以因應工業與住宅需求擴張。",
  "Transportation network": "交通運輸網絡",
  "Naka-Kyushu Cross Road extensions": "中九州橫斷道路延伸",
  "Four-segment expressway system extending from Ueki IC through Koshi to Ozu IC, running north of the semiconductor corridor. Connects western Kumamoto to the existing expressway network at Ozu IC. Segments range from under survey to under construction.":
    "四段式高速公路系統，從植木交流道經合志延伸至大津交流道，沿半導體走廊北側行經。連接西熊本與大津交流道既有高速公路網絡。各路段進度介於調查中至施工中。",
  "Total extension": "總延伸長度",
  "Road sections": "道路路段數",
  "Survey to construction": "調查至施工",
  "Route span": "路線範圍",
  Mixed: "混合進度",
  "Ueki IC to Ozu IC": "植木交流道至大津交流道",
  "New JR Hohi Line station": "JR 豊肥本線新站",
  "A new station is planned on the JR Hohi Line between Sanrigi and Haramizu stations in Kikuyo Town. The station will serve the Semicon Techno Park corridor and the adjacent TSMC factory area, providing direct rail access from Kumamoto City to the semiconductor cluster.":
    "菊陽町 JR 豊肥本線三里木至原水站之間規劃新設車站。該站將服務半導體科技園區走廊及鄰近的台積電廠區，提供從熊本市直達半導體聚落的鐵路交通。",
  "Station status": "車站狀態",
  "JR Hohi Line": "JR 豊肥本線",
  "Between stations": "介於車站之間",
  "Adjacent facility": "鄰近設施",
  "Sanrigi-Haramizu": "三里木-原水",
  "Semicon Techno Park": "半導體科技園區",
  "Kumamoto Airport access rail": "熊本機場聯絡鐵路",
  "Planned 6.8km rail extension from Kumamoto Station to Aso Kumamoto Airport via the JR Hohi Line corridor. Estimated travel time of 44 minutes (39 minutes with express service). Projected daily ridership of 4,900 to 5,500 passengers by 2035.":
    "從熊本站經 JR 豊肥本線走廊延伸 6.8 公里至阿蘇熊本機場的軌道規劃。預估行車時間 44 分鐘（快速列車 39 分鐘）。2035 年預估每日旅客量 4,900 至 5,500 人。",
  "Rail extension": "軌道延伸",
  "Estimated budget": "預估預算",
  "Kumamoto to airport": "熊本至機場",
  "Ridership forecast": "旅客量預估",
  "10-minute / 20-minute ring road concept": "10 分鐘 / 20 分鐘環狀道路構想",
  "Government vision for a metropolitan ring road system centered on Kumamoto. Three connection roads (north, south, and airport) form a ring enabling highway interchange access within 10 minutes and airport access within 20 minutes from any point in the metro area. Integrates Shinkansen, light rail, bus, and Park and Ride facilities. Source: 3rd Kumamoto Metropolitan Road Network Study Committee (December 2019).":
    "以熊本為中心的都會區環狀道路系統政府願景。北、南及機場三條連絡道路形成環狀，讓都會區內任何地點均可在 10 分鐘內到達高速公路交流道、20 分鐘內到達機場。整合新幹線、輕軌、公車及停車轉乘設施。資料來源：第三屆熊本都會區道路網絡研究委員會（2019 年 12 月）。",
  "IC access target": "交流道可達目標",
  "Airport access target": "機場可達目標",
  "North, south, airport": "北線、南線、機場線",
  "Rail, bus, Park and Ride": "鐵路、公車、停車轉乘",
  "Multi-modal": "多元運具",
  "Kumamoto transport overview": "熊本交通總覽",
  "Railway cost structure and community transport status across the Kumamoto metropolitan area. Facility maintenance accounts for 50% of railway operating costs, with direct transport at 39% and other costs at 11%. Community transport map shows suburban municipalities served by shared taxis and community buses, with many bus routes carrying only 0-7 passengers per vehicle per day, highlighting underserved corridors.":
    "熊本都會區鐵路成本結構與社區交通現況。設施維護占鐵路營運成本 50%，直接運輸占 39%、其他 11%。社區交通圖顯示郊區自治體以共乘計程車與社區巴士服務，許多公車路線每日每車僅載 0-7 名旅客，凸顯服務不足的走廊。",
  "Railway costs on maintenance": "鐵路維護成本占比",
  "Direct transport costs": "直接運輸成本占比",
  "Low-density bus routes": "低密度公車路線",
  "Municipalities mapped": "涵蓋自治體數",
  "Regional traffic flow changes": "區域交通流量變化",
  "Inter-regional traffic flow analysis from the Kumamoto Person Trip Survey comparing 2012 to 1997 baselines. Kikuyo-cho shows the highest growth at 1.67x, with the Kumamoto-to-Kikuyo corridor growing 1.54x. Overall Kumamoto metro traffic grew 1.13x to 1.67M trips/day. Commuting flows toward the northeast semiconductor cluster show the strongest growth, with Kikuyo-to-Ozu flows also at 1.67x.":
    "熊本都市交通調查的區域間交通流量分析，比較 2012 年與 1997 年基線。菊陽町成長最高達 1.67 倍，熊本至菊陽走廊成長 1.54 倍。熊本都會區整體交通量成長 1.13 倍至每日 167 萬人次。往東北側半導體聚落的通勤流量成長最為顯著，菊陽至大津亦達 1.67 倍。",
  "Kikuyo traffic growth": "菊陽交通成長",
  "Kumamoto-Kikuyo corridor": "熊本-菊陽走廊",
  "Kumamoto metro trips": "熊本都會旅次",
  "Survey period": "調查期間",
  "Commuting challenges in Ozu-machi": "大津町通勤挑戰",
  "Street-level view of road conditions near the semiconductor cluster in Ozu-machi. Single-lane roads with no sidewalks, shared with drainage infrastructure and bordered by residential apartment blocks. Illustrates the infrastructure gap between existing narrow roads and the traffic demands of the growing semiconductor workforce.":
    "大津町半導體聚落周邊的街道實況。單車道無人行道，與排水設施共用，兩側為住宅大樓。呈現既有狹窄道路與日益成長的半導體從業人員交通需求之間的基礎設施落差。",
  "Road width": "道路寬度",
  "Single lane": "單車道",
  "Sidewalk provision": "人行道設置",
  None: "無",
  "Area traffic growth": "區域交通成長",

  // Government zones evidence
  "Government zones": "政府區域",
  "Kumamoto Science Park": "熊本科學園區",
  "A distributed science park model connecting multiple industrial zones across Kumamoto. Five strategic pillars: advanced infrastructure, park management, talent development, startup incubation, and shared research facilities. Links university and industry collaboration at Semi-Con Techno Park, Kenei Industrial Park (Kikuchi), and Ozu-machi Industrial Park.":
    "連結熊本多個工業區的分散式科學園區模式。五大策略支柱：先進基礎設施、園區管理、人才發展、新創培育及共用研究設施。串聯半導體科技園區、縣營工業園區（菊池）及大津町工業園區的產學合作。",
  "Strategic pillars": "策略支柱",
  "Industrial zones": "工業區數",
  Distributed: "分散式",
  "Park model": "園區模式",
  "AI, Robotics": "AI、機器人",
  "Focus sectors": "重點領域",
  "Semiconductor cluster industrial parks": "半導體聚落工業園區",
  "Map of semiconductor cluster areas and industrial parks around JASM/TSMC. Shows Koshi municipal industrial park, Kikuyo Town-operated industrial park, Kikuyo rezoned development district, Ozu town-operated industrial zone, Kumamoto Prefecture industrial park in Kikuchi, and Kyokushikawabe business zone. Transport links include Higo-Ozu access road, airport monorail, and Naka-Kyushu Cross Road.":
    "JASM/台積電周邊半導體聚落區域與工業園區地圖。包含合志市營工業園區、菊陽町營工業園區、菊陽重劃開發區、大津町營工業區、菊池縣營工業園區及旭志川邊事業區。交通連結包括肥後大津聯絡道路、機場單軌及中九州橫斷道路。",
  "Rezoned development": "重劃開發",
  "Town-operated zone": "町營區域",
  "Prefecture park": "縣營園區",
  "TSMC area infrastructure and industrial parks":
    "台積電區域基礎設施與工業園區",
  "Overview of infrastructure status around TSMC expansion. Documents 13 planned industrial park sites across Kikuchi, Koshi, Ozu, Mashiki, and surrounding municipalities ranging from 8ha to 25ha, with opening dates from 2023 to 2028. Also summarizes four key infrastructure projects: Naka-Kyushu Cross Road, Ozu-Ueki Line road widening (2.4km to 6 lanes), airport access railway, and Kumamoto-Taiwan air routes.":
    "台積電擴建周邊的基礎設施現況概覽。記載 13 處規劃中工業園區，分布於菊池、合志、大津、益城及周邊自治體，面積 8 至 25 公頃，啟用日期自 2023 至 2028 年。同時摘要四項重大基礎建設：中九州橫斷道路、大津-植木線道路拓寬（2.4 公里擴至 6 車道）、機場聯絡鐵路及熊本-台灣航線。",
  "Planned industrial sites": "規劃中工業園區",
  "Site size range": "基地面積範圍",
  "Opening timeline": "啟用時程",
  "Key infrastructure projects": "重大基礎建設",
  "New grand airport concept": "新大空港構想",
  "Conceptual vision map for the New Grand Airport initiative. Identifies three development zones: north of airport (Kumamoto Science Park), south side of airport, and the Southern Prefecture Region. Shows strategic road connections to Fukuoka, Oita, Miyazaki, and Kagoshima via Kyushu Main Expressway, Central Kyushu Transversal Road, and Ariake Sea Coastal Road.":
    "新大空港構想概念願景圖。劃分三大開發區域：機場北側（熊本科學園區）、機場南側及南部縣域。展示經九州主要高速公路、中九州橫斷道路及有明海沿岸道路通往福岡、大分、宮崎及鹿兒島的策略性道路連結。",
  "Development zones": "開發區域",
  "Science Park zone": "科學園區區域",
  "Airport-side zone": "機場側區域",
  "Southern Prefecture": "南部縣域",
  Regional: "區域",
  North: "北側",
  South: "南側",
  "East Asian route strategy": "東亞航線策略",
  "Strategic plan for attracting East Asian air routes to Kumamoto Airport. Targets 4.86 million passengers by 2027 (up from 3.92M in 2022) and 6.22 million by 2051. International routes planned to grow from 4 in 2022 to 11 by 2027 and 17 by 2051. Cargo volume targets 27,000 tons by 2027 and 42,000 tons by 2051. Includes new passenger terminal building and route network to major East Asian cities.":
    "吸引東亞航線至熊本機場的策略計劃。目標 2027 年旅客量達 486 萬人次（2022 年為 392 萬），2051 年達 622 萬人次。國際航線規劃從 2022 年的 4 條增至 2027 年 11 條、2051 年 17 條。貨運量目標 2027 年 27,000 噸、2051 年 42,000 噸。包含新旅客航廈與通往東亞主要城市的航線網絡。",
  "Passenger target 2027": "2027 旅客目標",
  "International routes 2027": "2027 國際航線數",
  "Cargo target 2027": "2027 貨運目標",
  "Passenger target 2051": "2051 旅客目標",

  // Education pipeline evidence
  "Education pipeline": "教育管道",
  "University partnerships with JASM": "大學與 JASM 合作",
  "Regional university network feeding talent directly into JASM/TSMC operations. Multiple Kumamoto-area universities have established semiconductor engineering tracks, research partnerships, and internship pipelines linked to the JASM facility, creating a local talent supply chain for the expanding semiconductor cluster.":
    "區域大學網絡直接為 JASM/台積電輸送人才。多所熊本地區大學已建立半導體工程學程、研究合作及與 JASM 設施連結的實習管道，為擴張中的半導體聚落建構在地人才供應鏈。",
  "Primary employer link": "主要雇主連結",
  "Partner universities": "合作大學",
  Semiconductor: "半導體",
  "Engineering focus": "工程重點",
  "Regional pipeline": "區域管道",
  Multiple: "多所",
  "International education and public school support":
    "國際教育與公立學校支援",
  "Dual-track system supporting families of foreign semiconductor engineers. International school expansion: Kyushu Lutheran Academy launched an International Elementary division with IB curriculum in English-Japanese bilingual environment; Kumamoto International School (KIS) is expanding facilities and hiring Taiwanese teachers for Taiwanese families. Public school transformation: Kumamoto City Board of Education deploys Japanese language instructors and translators to public schools to help foreign children integrate into local districts. Under the Internationalization Promotion Vision, foreign engineer families are treated as long-term residents, strengthening bilingual adaptability in public education.":
    "支援外籍半導體工程師家庭的雙軌制度。國際學校擴建：九州路德學院新設國際小學部，提供 IB 課程的英日雙語環境；熊本國際學校（KIS）正在擴建設施並聘用台灣籍教師服務台灣家庭。公立學校轉型：熊本市教育委員會在公立學校配置日語指導人員與翻譯，協助外籍兒童融入在地學區。在「國際化推進ビジョン」下，外籍工程師家庭被視為長期居民，強化公共教育的雙語適應能力。",
  "Kyushu Lutheran Academy": "九州路德學院",
  "IB curriculum": "IB 課程",
  "Kumamoto International School": "熊本國際學校",
  Expanding: "擴建中",
  "Bilingual": "雙語",
  "Public school support": "公立學校支援",
  "Dual track": "雙軌制",
  "International and public": "國際與公立",
  "College graduate wage gap": "大學畢業生薪資差距",
  "Bloomberg analysis showing widening wage gap between college graduates and overall workforce. In 2023, college graduate wage increase rate reached 3.1% versus 2.1% in the Shunto (spring wage negotiations), with the gap accelerating sharply from 2022. Sources: Institute of Labour Administration (graduates), RENGO (Shunto).":
    "Bloomberg 分析顯示大學畢業生與整體勞動力的薪資差距持續擴大。2023 年大學畢業生薪資增幅達 3.1%，相較春鬥（春季薪資談判）的 2.1%，差距自 2022 年起加速擴大。資料來源：勞務行政研究所（畢業生）、連合（春鬥）。",
  "Graduate wage increase (2023)": "畢業生薪資增幅（2023）",
  "Shunto wage increase (2023)": "春鬥薪資增幅（2023）",
  "Gap widening": "差距擴大",
  "Trend period": "趨勢期間",

  // Semiconductor ecosystem evidence
  "Semiconductor ecosystem": "半導體生態系",
  "Kyushu semiconductor factories": "九州半導體工廠",
  "Map of major semiconductor-related factories across Kyushu. Semiconductor fabs (red): Mitsubishi Electric (Fukuoka), Sony (Nagasaki, Kumamoto x2, Kagoshima), Toshiba (Oita, Miyazaki), TSMC (Kumamoto), Renesas (Kumamoto), Tokyo Electron (Kumamoto), Rohm (near Saga). Materials and equipment makers (blue): SUMCO (Saga), Kyocera (Kagoshima). Company names include group companies.":
    "九州主要半導體相關工廠分布圖。晶圓廠（紅色）：三菱電機（福岡）、Sony（長崎、熊本 x2、鹿兒島）、東芝（大分、宮崎）、台積電（熊本）、瑞薩（熊本）、東京威力科創（熊本）、Rohm（佐賀周邊）。材料與設備商（藍色）：SUMCO（佐賀）、京瓷（鹿兒島）。",
  "Semiconductor fabs": "晶圓廠",
  "Prefectures covered": "涵蓋縣數",
  "Red markers": "紅色標記",
  "Blue markers": "藍色標記",
  Fabs: "晶圓廠",
  Materials: "材料",
  "TSMC area industrial park schedule": "台積電區域工業園區時程",
  "Detailed map and table of 13 planned industrial parks around TSMC. Sites span Kikuchi, Koshi, Ozu, Mashiki, Tamana, Yamaga, and Kikuchi municipalities. Includes Kikuchi Techno Park (25ha, public, FY2026), Semicon Techno Park West (11.2ha, FY2025), Ozu site on Route 325 (7.9ha, FY2027), and JASM Phase 1 factory site (25ha). Mix of public and private development from 2023 to 2028.":
    "台積電周邊 13 處規劃中工業園區的詳細地圖與表格。分布於菊池、合志、大津、益城、玉名、山鹿等自治體。包含菊池科技園區（25 公頃、公營、FY2026）、半導體科技園區西（11.2 公頃、FY2025）、325 號線大津用地（7.9 公頃、FY2027）及 JASM 第一期廠區（25 公頃）。2023 至 2028 年公私混合開發。",
  "Planned park sites": "規劃園區數",
  "Public site range": "公營用地範圍",
  "Opening schedule": "啟用時程",
  Municipalities: "自治體",
  "East Asia supply chain proximity": "東亞供應鏈鄰近性",
  "Supply chain logistics map showing Kyushu as Japan's semiconductor hub and gateway to East Asia. Proximity to Taiwan (foundry), Korea (memory), and Shanghai (end-market and supply chain hinterland). All three connections within 1-2 days by sea and under 3 hours by air, offering shorter logistics times and integration advantages.":
    "供應鏈物流圖呈現九州作為日本半導體樞紐與東亞門戶的地位。鄰近台灣（晶圓代工）、韓國（記憶體）及上海（終端市場與供應鏈腹地）。三者均在海運 1-2 天、空運 3 小時以內，具備更短的物流時效與整合優勢。",
  "Air to Taiwan/Korea/Shanghai": "飛往台灣/韓國/上海",
  "Sea freight": "海運時效",
  "Key supply chain partners": "主要供應鏈夥伴",
  "Kyushu gateway role": "九州門戶角色",
  Hub: "樞紐",

  // Investment analysis evidence
  "Investment analysis": "投資分析",
  "TSMC area population and land prices": "台積電區域人口與地價",
  "Population and land price trends around TSMC expansion. Ozu-machi land prices surged 33% (2023-2024), the highest increase nationally, rising from ¥43,000/sqm in 2014 to ¥77,000/sqm in 2024. Kikuyo-cho rose 30.8% to ¥95,600/sqm. TSMC corridor population has grown steadily, with Kikuyo reaching approximately 63,000 and showing 29% growth over the measured period.":
    "台積電擴建周邊的人口與地價趨勢。大津町地價飆升 33%（2023-2024），為全國最高漲幅，從 2014 年每平方公尺 43,000 日圓升至 2024 年 77,000 日圓。菊陽町上漲 30.8% 至每平方公尺 95,600 日圓。台積電走廊人口穩定成長，菊陽達約 63,000 人，觀測期間成長 29%。",
  "Ozu land price surge (23-24)": "大津地價飆漲（23-24）",
  "Ozu land price (2024)": "大津地價（2024）",
  "Kikuyo land price surge": "菊陽地價飆漲",
  "National increase rate": "全國漲幅排名",
  "Rental assessment (4LDK/115sqm)": "租金評估（4LDK/115 平方公尺）",
  "AI rent assessment for A-type 4LDK/115.1sqm property. Assessed rent: ¥170,000/month (¥1,477/sqm). Based on 30 comparable properties corrected to same conditions. Grade distribution: high ¥243K-259K (4 units), slightly high ¥208K-243K (4 units), average ¥174K-208K (13 units), slightly low ¥139K-174K (8 units). Correction factors include building age, floor area, walking distance, and amenities.":
    "A 型 4LDK/115.1 平方公尺物件 AI 租金評估。評估租金：每月 170,000 日圓（每平方公尺 1,477 日圓）。依據 30 筆可比物件校正至同等條件。等級分布：高 243K-259K（4 筆）、略高 208K-243K（4 筆）、平均 174K-208K（13 筆）、略低 139K-174K（8 筆）。校正因子包含屋齡、樓地板面積、步行距離及設施。",
  "Assessed monthly rent": "評估月租金",
  "Property type": "物件類型",
  "Floor area": "樓地板面積",
  "Comparable properties": "可比物件數",
  "Rental assessment (4LDK/89sqm)": "租金評估（4LDK/89 平方公尺）",
  "AI rent assessment for A-type 4LDK/89.1sqm property. Assessed rent: ¥160,000/month (¥1,796/sqm). Notes that comparable house rentals are rare in the area, and new construction in Ozu-machi is increasing with intense competition. Grade distribution: slightly high ¥196K-233K (8 units), average ¥158K-196K (8 units), slightly low ¥121K-158K (12 units).":
    "A 型 4LDK/89.1 平方公尺物件 AI 租金評估。評估租金：每月 160,000 日圓（每平方公尺 1,796 日圓）。備註該區域可比透天出租案例稀少，大津町新建案增加且競爭激烈。等級分布：略高 196K-233K（8 筆）、平均 158K-196K（8 筆）、略低 121K-158K（12 筆）。",
  "Unit rate": "單價",
  "Acquisition loan analysis": "取得貸款分析",
  "Loan and yield analysis for existing property renovation scenarios at three price points (¥42M, ¥44M, ¥46M). Purchase incurs 7% fees; 1-2 months preparation with furnished rental fitout. Gross yield ranges from 4.26-5.22% depending on price and rent tier. Loan terms: 50% LTV, 20-year term, 2.50% interest rate, with monthly payments from ¥111,280 to ¥121,878.":
    "三種價格情境（4,200 萬、4,400 萬、4,600 萬日圓）的既有物件翻修貸款與收益分析。購入費用 7%；整備期 1-2 個月含帶家具出租裝修。毛收益率依價格與租金等級介於 4.26-5.22%。貸款條件：50% LTV、20 年期、2.50% 利率，月還款 111,280 至 121,878 日圓。",
  "Gross yield range": "毛收益率範圍",
  "Loan-to-value": "貸款成數",
  "Interest rate": "利率",
  "Loan term": "貸款期限",
  "Property P&L comparison (Ozu and Kikuyo)":
    "物件損益比較（大津與菊陽）",
  "Side-by-side profit and loss analysis for two properties in Ozu-machi and Kikuyo-machi. Total acquisition costs of 32.6-33.9M yen per unit including 7% purchase fees, 2-3M renovation, and 3M furniture/appliance package. Sale prices of 42-43M yen yield pre-tax profit of 4.0-4.4M yen per property (2.6-2.9M after 35% tax). Gross rental yields range from 4.40% to 5.16% based on assessed rents of 154K-185K/month.":
    "大津町與菊陽町兩筆物件的損益並列分析。每戶總取得成本 3,260-3,390 萬日圓，含 7% 購入費用、200-300 萬翻修及 300 萬家具/家電配套。售價 4,200-4,300 萬日圓可產生稅前利潤 400-440 萬日圓（扣 35% 稅後 260-290 萬）。依評估月租金 154K-185K 計算毛收益率 4.40% 至 5.16%。",
  "Sale price per unit": "每戶售價",
  "Pre-tax profit": "稅前利潤",
  "Tax rate applied": "適用稅率",

  // =====================================================================
  // Utility strings (utils.js)
  // =====================================================================

  "Just now": "剛才",
  "days ago": "天前",
  "hours ago": "小時前",
  day: "天",
  days: "天",
  hour: "小時",
  hours: "小時",
  ago: "前",

  // =====================================================================
  // Step-handlers.js UI strings
  // =====================================================================

  "Development zone": "開發區域",
  "Road infrastructure": "道路基礎設施",
  "Select a cluster or road on the map to view details.":
    "點選地圖上的聚落或道路以檢視詳情。",
  Clusters: "聚落",

  // =====================================================================
  // Miscellaneous UI strings
  // =====================================================================

  "Kumamoto Prefectural Government designated semiconductor development zone with tax incentives, streamlined permitting, and infrastructure investments.":
    "熊本縣政府劃定的半導體開發區，提供租稅優惠、簡化審批及基礎建設投資。",
  "Avg appreciation": "平均增值率",

  // =====================================================================
  // Step-handlers.js additional strings
  // =====================================================================

  "Development zones": "開發區域",
  "Supporting evidence": "佐證資料",
  "Silicon triangle": "矽三角",
  "Natural advantages verified": "自然優勢已驗證",
  "4T+ yen government commitment": "逾 4 兆日圓政府承諾",
  "investment properties": "投資物件",
  "Combined 5-year return": "五年合計報酬",
  "across the portfolio": "橫跨投資組合",
  "Enter Q&A": "進入問答",
  "Select an item below to view detailed documentation.":
    "請選擇下方項目以檢視詳細文件。",
  "View all evidence": "檢視所有佐證資料",
  "Government levels": "政府層級",

  // =====================================================================
  // UI: cards.js
  // =====================================================================

  "Corporate investment": "企業投資",
  "Investment comparison": "投資比較",
  "Total corporate investment in the Kumamoto semiconductor corridor.":
    "熊本半導體走廊的企業投資總額。",
  "Total investment": "投資總額",
  "Investment data from official company announcements":
    "資料來源：各企業官方公告",

  // =====================================================================
  // UI: core.js
  // =====================================================================

  "Reopen AI chat": "重新開啟 AI 聊天",
  "Reopen guide": "重新開啟導覽",
  "Go back": "返回",
  "Back to map": "返回地圖",
  Document: "文件",
  Gallery: "圖庫",
  Source: "來源",
  Evidence: "佐證資料",
  "Dashboard opened:": "儀表板已開啟：",
  "Dashboard closed": "儀表板已關閉",
  Continue: "繼續",
  "Future outlook": "未來展望",
  "See the 2030+ completed state: science park expansion, grand airport, road completions, and new stations.":
    "檢視 2030+ 完成狀態：科學園區擴張、大空港、道路完工及新站。",

  // =====================================================================
  // UI: data-layers.js
  // =====================================================================

  "Water resources": "水資源",
  "Power resources": "電力資源",
  "Strategic location": "策略位置",
  "Government support": "政府支持",
  "Science park & airport concept": "科學園區與機場概念",
  "Education and talent pipeline": "教育與人才管道",
  "Rail commute": "鐵路通勤",
  "Infrastructure plan": "基礎建設計劃",
  "Property investment zones": "不動產投資區域",
  "Investment properties": "投資物件",
  Locations: "地點",
  "Kyushu energy facilities": "九州能源設施",
  Solar: "太陽能",
  Wind: "風力",
  Nuclear: "核能",
  "Data layer": "資料圖層",
  "Data layers": "資料圖層",
  "Active layers": "使用中圖層",
  "Investment properties in the Kumamoto corridor.":
    "熊本走廊內的投資物件。",
  "Active routes": "現行航線",
  "Total destinations": "目的地總數",
  "Q&A mode": "問答模式",
  "Layer details": "圖層詳情",
  "No details available for this layer.": "此圖層暫無詳細資訊。",
  "JASM ESG report": "JASM ESG 報告",
  "Green power and sustainability section": "綠能與永續發展專區",
  "JASM ESG report - green power and sustainability section":
    "JASM ESG 報告 - 綠能與永續發展專區",
  "layer hidden": "圖層已隱藏",
  "layer shown": "圖層已顯示",
  "Toggle": "切換",
  "on the map": "於地圖上",

  // =====================================================================
  // UI: overlays.js
  // =====================================================================

  "Previous property": "上一筆物件",
  "Next property": "下一筆物件",
  "Previous image": "上一張圖片",
  "Next image": "下一張圖片",
  "No preview available": "暫無預覽",
  "Document preview not yet available.": "文件預覽尚未提供。",
  "Open source document": "開啟來源文件",
  "Japanese property investment made easy.": "日本不動產投資，輕鬆上手。",
  "Previous item": "上一個項目",
  "Next item": "下一個項目",
  Close: "關閉",
  Previous: "上一頁",
  Next: "下一頁",
  "Download summary": "下載摘要",
  "Kumamoto investment summary": "熊本投資摘要",
  "Greater Kumamoto Technology Corridor — presentation summary":
    "大熊本科技走廊 — 簡報摘要",
  "Fund overview": "基金概覽",
  "Science Park corridor": "科學園區走廊",
  "Corporate partners": "企業夥伴",
  Property: "物件",
  Type: "類型",
  "Drive to JASM": "到 JASM 車程",
  "Floor Area": "樓地板面積",
  "This summary is for informational purposes only and does not constitute investment advice.":
    "本摘要僅供參考，不構成投資建議。",
  "Generated on": "產出日期",
  Company: "企業",
  Sector: "產業",
  Investment: "投資額",
  Employees: "員工人數",

  // =====================================================================
  // UI: inspector.js
  // =====================================================================

  "Kumamoto corridor": "熊本走廊",
  "Supporting documents": "佐證文件",

  // =====================================================================
  // UI: evidence.js
  // =====================================================================

  "Evidence library": "佐證資料庫",
  "Explore detailed evidence and documentation for each category.":
    "瀏覽各類別的詳細佐證資料與文件。",
  item: "項",
  items: "項",

  // =====================================================================
  // UI: ai-chat.js
  // =====================================================================

  "Water quality?": "水質？",
  "What about Kumamoto's water quality and supply?":
    "熊本的水質與供水狀況如何？",
  "Government support?": "政府支持？",
  "How strong is government support for the semiconductor corridor?":
    "政府對半導體走廊的支持力度如何？",
  "Property returns?": "物件報酬？",
  "What are the projected property returns in Kumamoto?":
    "熊本的預期不動產報酬率為何？",
  "Talent pipeline?": "人才管道？",
  "Tell me about the talent pipeline and workforce availability.":
    "請介紹人才管道與勞動力供給情況。",
  "Key risks to consider:": "需考量的主要風險：",
  "Natural disaster:": "天然災害：",
  "The 2016 Kumamoto earthquake caused significant damage. Seismic building codes have since been upgraded.":
    "2016 年熊本地震造成重大損害，地震建築法規已全面升級。",
  "Construction delays:": "施工延遲：",
  "TSMC's second fab timeline could shift, affecting demand projections.":
    "台積電第二座晶圓廠時程可能調整，影響需求預估。",
  "Currency exposure:": "匯率風險：",
  "Yen-denominated assets carry FX risk for international investors.":
    "日圓計價資產對國際投資人存在匯率風險。",
  "Liquidity:": "流動性：",
  "Regional Japanese real estate is less liquid than metro markets.":
    "日本地方不動產的流動性低於都會市場。",
  "However, the ¥4T+ government commitment and TSMC's operational first fab provide strong downside protection.":
    "然而，逾 4 兆日圓的政府承諾與台積電已投產的第一座晶圓廠提供強勁的下檔保護。",
  "The GKTK Fund offers a structured entry point:":
    "GKTK 基金提供結構化進場方式：",
  "Schedule a consultation": "預約諮詢",
  "Review the fund prospectus and individual property details":
    "審閱基金公開說明書及個別物件資料",
  "Complete KYC and accreditation verification":
    "完成 KYC 及合格投資人驗證",
  "Fund commitment with quarterly capital calls":
    "基金承諾，按季資本催繳",
  "Minimum investment starts at ¥50M. Would you like to schedule a call with an advisor?":
    "最低投資金額 5,000 萬日圓起。是否需要預約顧問諮詢？",
  "Japan committed ¥3.9 trillion to rebuild domestic chip production as part of national economic security strategy.":
    "日本承諾投入 3.9 兆日圓重建國內晶片產能，作為國家經濟安全戰略的一環。",
  "TSMC's Kumamoto fab is the centerpiece—bringing cutting-edge manufacturing back to Japan.":
    "台積電熊本廠是核心 — 將尖端製造帶回日本。",
  "Kumamoto was selected for three reasons: ultra-pure water from the Aso aquifer, reliable power infrastructure, and an existing semiconductor ecosystem with Sony's facility.":
    "熊本獲選的三大原因：阿蘇含水層的超純水、可靠的電力基礎設施，以及 Sony 設施所構建的既有半導體生態系。",
  "These natural advantages were decisive for chip fabrication requirements.":
    "這些自然優勢對晶片製造需求而言具有決定性。",
  "JASM is TSMC's joint venture with Sony and Denso. The first fab began production in 2024 with 22/28nm chips.":
    "JASM 是台積電與 Sony 及 Denso 的合資企業。第一座晶圓廠於 2024 年以 22/28 奈米製程投產。",
  "A second fab for 6nm chips is under construction—¥2 trillion total investment creating 3,400+ jobs.":
    "第二座 6 奈米晶圓廠建設中 — 總投資 2 兆日圓，創造逾 3,400 個工作機會。",
  "Land prices have appreciated 15-25% annually since TSMC's announcement.":
    "自台積電宣布設廠以來，地價每年增值 15-25%。",
  "Kikuyo and Ozu areas see the strongest growth. Residential land near JASM has roughly doubled since 2021.":
    "菊陽與大津地區增幅最為顯著，JASM 周邊住宅用地自 2021 年以來幾近翻倍。",
  "Traffic congestion has increased with JASM construction. The prefecture is investing ¥50 billion in infrastructure upgrades.":
    "JASM 建設期間交通壅塞加劇，縣政府投入 500 億日圓進行基礎建設升級。",
  "New expressway connections and the planned Kumamoto Airport rail link will reduce commute times.":
    "新高速公路連結及熊本機場聯絡鐵路規劃將縮短通勤時間。",
  "Properties show 4-6% gross rental yields with projected appreciation of 8-15% annually through 2030.":
    "物件呈現 4-6% 毛租金收益率，預估至 2030 年每年增值 8-15%。",
  "Properties within 15 minutes of JASM command premium rents from engineers and technicians.":
    "距 JASM 15 分鐘內的物件可向工程師與技術人員收取溢價租金。",
  "The Kumamoto corridor is experiencing unprecedented transformation with over ¥4 trillion in committed investment.":
    "熊本走廊正經歷空前的轉型，已承諾投資逾 4 兆日圓。",
  "I can help with land prices, infrastructure plans, or specific investment opportunities.":
    "我可以協助您了解地價、基礎建設計劃或特定投資機會。",
  "Based on Q3 2024 data": "資料基準：2024 年第三季",
  "I'd like to schedule a call with an advisor.":
    "我想預約顧問諮詢。",
  "I'll connect you with one of our investment advisors. They'll reach out within 24 hours to discuss your specific interests.":
    "我將為您安排一位投資顧問。顧問將在 24 小時內與您聯繫，討論您的具體需求。",
  "What's the best way to reach you?":
    "請問最方便的聯繫方式是？",
  "I'd like to download a summary of this presentation.":
    "我想下載這份簡報的摘要。",
  "Your summary has been downloaded. It includes the fund overview, corporate partners, and property details we reviewed.":
    "您的摘要已下載完成，內含基金概覽、企業夥伴及我們檢視過的物件詳情。",

  // =====================================================================
  // UI: charts.js
  // =====================================================================

  "View data as table": "以表格檢視資料",
  "Annual appreciation": "年增值率",
  "Annual rent": "年租金",
  "Net Profit (5yr)": "淨利潤（五年）",
  Bear: "保守",
  Average: "中性",
  Bull: "樂觀",
  "Growth drivers": "成長動能",
  "Truth engine": "真相引擎",
  "Key factors driving future value appreciation for this property:":
    "驅動此物件未來增值的關鍵因素：",
  "Performance calculator": "績效計算器",
  "Financial projection": "財務預測",
  "Appreciation rate": "增值率",
  "Est. selling price (5yr)": "預估售價（五年）",
  Range: "區間",
  "Rental yield": "租金收益率",
  "Annual rental income": "年租金收入",
  "Applicable taxes": "適用稅費",
  "Net profit (5yr)": "淨利潤（五年）",
  "Sample data": "範例資料",
  "Price data from Kumamoto Land Registry (Jan 2026)":
    "價格資料來源：熊本不動產登記處（2026 年 1 月）",
  "Rental estimates based on local property managers":
    "租金估算依據：在地物管業者",
  "View rental report": "檢視租金報告",
  "Area statistics": "區域統計",
  "Market overview": "市場概覽",
  "Avg. annual appreciation": "平均年增值率",
  "Avg. rental yield": "平均租金收益率",
  "Appreciation trend": "增值趨勢",
  "Year-over-year property appreciation in the Kumamoto semiconductor corridor.":
    "熊本半導體走廊逐年不動產增值率。",
  "Data from Kumamoto Prefecture Real Estate Association":
    "資料來源：熊本縣不動產協會",
  "% appreciation": "% 增值",
  "Investment (¥B)": "投資額（十億日圓）",
  High: "高",
  Medium: "中",
  "Scenario comparison": "情境比較",
  "Historical property appreciation rates in the Kumamoto semiconductor corridor":
    "熊本半導體走廊歷史不動產增值率",
  "Corporate investment comparison in the Kumamoto semiconductor corridor":
    "熊本半導體走廊企業投資比較",
  "Q1 2026": "2026 年第一季",
  "projected": "預估",
  "for bear, average, and bull market conditions":
    "保守、中性及樂觀市場情境",
  "showing": "呈現",

  // =====================================================================
  // UI: inspector.js
  // =====================================================================

  "Resources and location": "資源與區位",
  Overview: "概覽",
  "Semiconductor talent pipeline": "半導體人才管道",
  "Semiconductor employment": "半導體就業",
  "Employment data": "就業數據",
  "Return calculator": "報酬計算器",
  Yield: "收益率",
  "Rental demand forecast": "租賃需求預測",
  "Pipeline connection": "管道連結",
  Status: "狀態",
  "Commute saved": "通勤節省",
  Completion: "完工",
  Budget: "預算",
  Distance: "距離",
  "Drive time": "車程時間",
  "Monthly rent": "月租金",
  "Guaranteed yield": "保證收益率",
  "Market yield": "市場收益率",
  "Annual rent": "年租金",
  IRR: "IRR",
  NOI: "NOI",
  "Projected annual income": "預估年收入",
  "View evidence": "檢視佐證資料",
  "View source document": "檢視來源文件",
  "View all routes": "檢視所有航線",
  Avg: "中性",
  "Semiconductor factory": "半導體工廠",
  "Rail connection": "軌道連結",
  "Science park circles": "科學園區範圍",
  "Airport access": "機場聯絡",
  "No government data available.": "暫無政府資料。",
  "No images available.": "暫無圖片。",
  "No truth engine data available.": "暫無真相引擎資料。",
  Image: "圖片",
  Timeline: "時程",
  "Zone profile": "區域概況",
  Risk: "風險",
  Moderate: "中等",
  "Kumamoto's natural advantages": "熊本的自然優勢",
  "feeds directly into the Kumamoto semiconductor corridor":
    "直接輸送至熊本半導體走廊",
  Scenario: "情境",
  Year: "年度",
  Appreciation: "增值",
  Details: "詳情",
  Case: "情境",
  confidence: "信心水準",
  "Bar chart comparing investment scenarios": "投資情境比較長條圖",
  "Line chart showing appreciation trend": "增值趨勢折線圖",

  // =====================================================================
  // UI: cards.js
  // =====================================================================

  Capacity: "容量",
  "View all evidence": "檢視所有佐證資料",
  "Connection details": "連線詳情",
  "Toggle layer": "切換圖層",
  "No data available": "暫無資料",

  // =====================================================================
  // Missing keys: inspector.js, cards.js, core.js, data-layers.js, etc.
  // =====================================================================

  // --- Inspector / panel labels ---
  "2030+ vision": "2030+ 願景",
  "Active commitments": "執行中的承諾",
  "Air access": "航空交通",
  Airlines: "航空公司",
  "Combined 5-year potential": "五年綜合潛力",
  "Government commitment": "政府承諾",
  "Government tier": "政府層級",
  "Infrastructure access": "基礎建設交通",
  "International routes": "國際航線",
  "International route": "國際航線",
  "Investment zones": "投資區域",
  "Zone overview": "區域概覽",
  "Zone details": "區域詳情",
  "Talent pipeline": "人才管道",
  "Water quality evidence": "水質佐證",
  "Government zone clusters": "政府區域集群",
  "Tri-level alignment": "三級政府協調",
  "National to local alignment": "中央至地方協調",
  "Key initiatives": "重點計畫",
  "Infrastructure timeline": "基礎建設時程",
  "Future outlook evidence": "未來展望佐證",
  "Future road network": "未來道路網絡",
  "Event history": "事件歷程",
  "Mitigation investments": "減災投資",
  "Energy mix": "能源組成",
  "Sustainable energy": "永續能源",
  "Airport master plan": "機場總體規劃",
  "Airport to city railway": "機場至市區鐵路",
  "New railway system": "新鐵路系統",
  "Roads and interchanges": "道路與交流道",
  "Planned infrastructure extension": "規劃中的基礎建設延伸",
  "Infrastructure roads": "基礎建設道路",
  "Corporate investment comparison chart": "企業投資比較圖",
  "Two demand waves": "兩波需求",
  "Supply gap": "供給缺口",
  "Commute to JASM": "通勤至 JASM",

  // --- Property / fund ---
  Properties: "物件",
  Property: "物件",
  properties: "物件",
  property: "物件",
  "Product type": "產品類型",
  "Target tenants": "目標租客",
  "Projected 5-year return": "預計五年報酬",
  "Projected return across": "預計報酬涵蓋",
  "Fund data unavailable.": "基金資料暫無。",
  "No properties yet.": "尚無物件。",
  "Five properties across three investment zones in the semiconductor corridor. Toggle a zone to explore its properties.": "半導體廊道三大投資區域共五處物件。切換區域以探索其物件。",
  "Click the property marker to explore details": "點擊物件標記以探索詳情",
  "in this zone": "在此區域",
  investment: "投資",
  combined: "合計",
  "View detailed breakdown": "檢視詳細分析",

  // --- Chart / scenario ---
  Scenario: "情境",
  "case scenario": "情境",
  scenario: "情境",
  Bear: "保守",
  Average: "平均",
  Bull: "樂觀",
  "Bar chart comparing corporate investments across seven companies in the Kumamoto corridor": "熊本廊道七家企業投資比較長條圖",
  "Bar chart comparing investment scenarios: Bear, Average, and Bull net profit": "投資情境比較長條圖：保守、平均與樂觀淨利",
  "Scenario comparison showing projected returns for bear, average, and bull market conditions": "情境比較顯示保守、平均與樂觀市況的預計報酬",
  IRR: "內部報酬率",
  NOI: "淨營業收入",
  Yield: "收益率",
  "Avg rental yield": "平均租金收益率",
  Occupancy: "入住率",
  Appreciation: "增值",
  Range: "範圍",
  "Range:": "範圍：",

  // --- Companies ---
  TSMC: "台積電",
  JASM: "JASM",
  Samsung: "三星",
  Sony: "索尼",
  Rohm: "羅姆",
  SUMCO: "SUMCO",
  Toshiba: "東芝",
  Kyocera: "京瓷",
  "China Airlines": "中華航空",
  "Korean Air": "大韓航空",
  "Starlux Airlines": "星宇航空",
  "Eastar Jet": "易斯達航空",
  "T'way Air": "德威航空",

  // --- Locations ---
  Kumamoto: "熊本",
  "Kumamoto Station": "熊本站",
  Fukuoka: "福岡",
  Oita: "大分",
  Kikuyo: "菊陽",
  Koshi: "合志",
  Ozu: "大津",
  Taiwan: "台灣",
  Korea: "韓國",
  Kaohsiung: "高雄",
  Kitakyushu: "北九州",

  // --- General labels ---
  Airport: "機場",
  Budget: "預算",
  City: "市",
  Clusters: "集群",
  Close: "關閉",
  Company: "企業",
  Completion: "完工",
  Concept: "概念",
  Connection: "連線",
  "Connection from": "連線自",
  Continue: "繼續",
  Details: "詳情",
  "Direct connections to": "直飛連結至",
  "Direct flight time": "直飛時間",
  "Direct flights from": "直飛航線自",
  Distance: "距離",
  Distribution: "分布",
  Document: "文件",
  Employees: "員工",
  Employment: "就業",
  Established: "成立",
  Evidence: "佐證資料",
  "Evidence report": "佐證報告",
  Gallery: "圖庫",
  High: "高",
  Housing: "住宅",
  Image: "圖片",
  Implementation: "實施",
  Infrastructure: "基礎建設",
  Investment: "投資",
  Jobs: "職缺",
  Landmark: "地標",
  Length: "長度",
  Light: "輕型",
  Location: "位置",
  Locations: "位置",
  Logistics: "物流",
  Low: "低",
  Medium: "中",
  Model: "模型",
  Moderate: "中等",
  Municipalities: "市町村",
  Next: "下一步",
  Nuclear: "核能",
  "Nuclear energy": "核能",
  "Nuclear power": "核能發電",
  Opening: "開業",
  Opportunity: "機會",
  Overview: "概覽",
  Planned: "規劃中",
  Prefecture: "縣",
  Previous: "上一步",
  Region: "區域",
  Resources: "資源",
  Risk: "風險",
  Road: "道路",
  Role: "角色",
  "Science Park": "科學園區",
  School: "學校",
  Scope: "範圍",
  Sector: "產業別",
  Seismic: "地震",
  Solar: "太陽能",
  "Solar power": "太陽能發電",
  Source: "來源",
  Station: "車站",
  Status: "狀態",
  Step: "步驟",
  Target: "目標",
  Timeline: "時程",
  Type: "類型",
  Universities: "大學",
  Uptime: "運轉率",
  Vibrancy: "活力",
  View: "檢視",
  view: "檢視",
  Vintage: "年份",
  Volcanic: "火山",
  Warehouse: "倉儲",
  Wind: "風力",
  "Wind energy": "風力能源",
  Year: "年度",
  Zones: "區域",
  Adjacent: "鄰近",
  Available: "可用",
  Avg: "平均",
  Doctorate: "博士",
  Flood: "洪水",
  Instructors: "講師",
  Kyutech: "九州工業大學",
  Medical: "醫療",
  Researchers: "研究人員",
  TBD: "待定",
  drive: "車程",
  exterior: "外觀",
  item: "項目",
  items: "項目",
  main: "主要",
  planned: "規劃中",
  secondary: "次要",
  trend: "趨勢",
  yr: "年",
  of: "的",

  // --- Time ---
  "day ago": "天前",
  "hour ago": "小時前",

  // --- Phrases and sentences ---
  "destinations across Korea, Taiwan, and greater Asia.": "橫跨韓國、台灣及亞洲主要城市的航點。",
  "connect Kumamoto to major semiconductor hubs across Asia.": "連結熊本與亞洲主要半導體樞紐。",
  "feeds directly into the Kumamoto semiconductor corridor, supplying graduates and researchers to JASM and other employers in the cluster.": "直接為熊本半導體廊道輸送畢業生與研究人員至 JASM 及廊道內其他企業。",
  "with our Kumamoto specialists": "與我們的熊本專家團隊",
  "levels active": "層級啟用中",
  "commute saved": "通勤節省",
  "Click any tier for details.": "點擊任一層級查看詳情。",
  "10-minute ring road": "十分鐘環狀道路",
  "Toggle layers to explore the completed state of the semiconductor corridor.": "切換圖層以探索半導體廊道的完成狀態。",
  "Service suspended": "航線暫停",
  "Flight time when active": "營運時飛行時間",
  "Shinkansen 30-minute connection to Hakata": "新幹線三十分鐘連結博多",
  "Supply catches up to demand around 2028-2029": "供給約在 2028-2029 年趕上需求",
  "Wave 1 now (construction + early Fab 1 occupants), wave 2 ~2027 (Fab 2 brings higher-income engineers with higher housing quality expectations)": "第一波需求為現階段（建設期 + Fab 1 早期進駐人員），第二波約 2027 年（Fab 2 帶來薪資更高且對居住品質有更高期待的工程師）",
  "No employment data available.": "暫無就業資料。",
  "No evidence documents available.": "暫無佐證文件。",
  "No forecast data available.": "暫無預測資料。",
  "No future outlook data available.": "暫無未來展望資料。",
  "No timeline data available.": "暫無時程資料。",
  "Enter Q&amp;A": "進入問答",
  "First wafers produced": "首批晶圓產出",
  "Second fab under construction": "第二座晶圓廠建設中",
  "TSMC Japan announced": "台積電日本宣布設廠",
  "National semiconductor strategy launched": "國家半導體戰略啟動",
  "Kumamoto earthquake": "熊本地震",
  "M7.3, significant infrastructure damage": "規模 7.3，基礎建設嚴重損毀",
  "Phase 1 groundbreaking": "第一期動工",
  "Phase 2 construction": "第二期建設",
  "METI, July 2024": "經濟產業省，2024 年 7 月",

  // --- Long descriptions ---
  "A new 6.8km rail connection will link Aso Kumamoto Airport directly to the JR Hohi Line, with an estimated travel time of 44 minutes from Kumamoto Station.": "全新 6.8 公里鐵路連線將把阿蘇熊本機場直接連結至 JR 豐肥本線，預計從熊本站出發約 44 分鐘可達。",
  "A proposed expansion of Aso Kumamoto Airport to serve as a regional semiconductor logistics hub.": "擬議擴建阿蘇熊本機場，使其成為區域半導體物流樞紐。",
  "A second fab for 6nm chips is under construction - ¥2 trillion total investment creating 3,400+ jobs.": "第二座晶圓廠專攻 6 奈米製程，目前建設中 - 總投資額達 2 兆日圓，創造逾 3,400 個就業機會。",
  "Japan's semiconductor strategy is backed by coordinated investment across central, prefectural, and local government - a rare tri-level alignment that de-risks the corridor.": "日本半導體戰略獲得中央、縣級與地方政府的協調投資支持 - 罕見的三級政府協調大幅降低廊道風險。",
  "TSMC's Kumamoto fab is the centerpiece - bringing cutting-edge manufacturing back to Japan.": "台積電熊本晶圓廠為核心 - 將尖端製造帶回日本。",
  "Kumamoto's natural advantages: water, power, and strategic position in the semiconductor supply chain.": "熊本的天然優勢：水資源、電力供應，以及在半導體供應鏈中的戰略位置。",
  "Major manufacturers chose Kumamoto for water quality - proof the resource meets industrial standards.": "主要製造商因水質選擇熊本 - 證明該資源符合工業標準。",
  "Kyushu leads Japan in renewable energy adoption, providing the stable and diverse power mix semiconductor manufacturing requires.": "九州在日本再生能源採用率領先，提供半導體製造所需的穩定多元電力組合。",
  "Kyushu's diverse energy mix powers the semiconductor corridor with solar, wind, and nuclear baseload - ensuring the stable 24/7 supply fabs require.": "九州多元的能源組成以太陽能、風力及核能基載電力支撐半導體廊道 - 確保晶圓廠所需的全天候穩定供電。",
  "Airport access infrastructure connecting the semiconductor corridor to Aso Kumamoto Airport.": "連接半導體廊道與阿蘇熊本機場的機場聯外基礎建設。",
  "Greater Kumamoto Technology Corridor - presentation summary": "大熊本科技廊道 - 簡報摘要",

  // --- Property specs ---
  "117.62 sqm (approx. 35.6 tsubo)": "117.62 平方公尺（約 35.6 坪）",
  "200.7 sqm (approx. 60.7 tsubo)": "200.7 平方公尺（約 60.7 坪）",
  "201.6 sqm (approx. 60.98 tsubo)": "201.6 平方公尺（約 60.98 坪）",
  "210.86 sqm (approx. 63.78 tsubo)": "210.86 平方公尺（約 63.78 坪）",
  "224.88 sqm (approx. 68.02 tsubo)": "224.88 平方公尺（約 68.02 坪）",
  "94.81 sqm total, 2 floors": "總面積 94.81 平方公尺，2 層樓",
  "4LDK, 3 parking spaces": "4LDK，3 個停車位",
  "3-4 spaces (planned)": "3-4 個車位（規劃中）",
  "4-5 spaces (planned)": "4-5 個車位（規劃中）",
  "Planned (spec to be confirmed)": "規劃中（規格待確認）",
  "6-9 months after contract signing": "簽約後 6-9 個月",
  "July 2026 onwards": "2026 年 7 月起",
  "June 2026 onwards": "2026 年 6 月起",
  "May 2026 onwards": "2026 年 5 月起",
  "Haramizu Land 1": "原水用地 1",
  "Haramizu Land 2": "原水用地 2",
  "Corporate RC condominiums and 3-4LDK detached houses": "企業 RC 大樓型公寓及 3-4LDK 獨棟住宅",
  "High-spec detached house rentals or renovations upgraded to expatriate standard": "高規格獨棟出租住宅或翻修至外派人員標準",
  "RC mansion condominiums and accommodation": "RC 大樓型公寓及宿舍",
  "Kyushu-level business support center, suitable for Japanese corporate senior executive families": "九州等級商務支援中心，適合日本企業高階主管家庭",
  "Large retail drives lifestyle density but internationalized services (bilingual clinics, international preschools) still being built out": "大型零售帶動生活機能密度，但國際化服務（雙語診所、國際幼兒園）仍在建設中",
  "Single engineers, long-term secondees, short-to-medium-term secondees": "單身工程師、長期派駐人員、中短期派駐人員",

  // --- Step jumper ---
  "Combined commitment": "合計承諾",
  "Commitment details": "承諾詳情",
  "JASM construction begins": "JASM 開始建設",
  appreciation: "增值",

  "Entry into map": "進入地圖",
  "Kumamoto investment guide": "熊本投資指南",
  "Start Journey": "開始導覽",
  "2d": "2D",

  // --- CSS/code false positives to skip ---
  "ga-airport-access": "ga-airport-access",
  "button, a, input, select, textarea": "button, a, input, select, textarea",
};
